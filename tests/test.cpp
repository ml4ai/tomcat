...
options_description load_options() {
    ...
    options.add_options()
            ...
            ("mission",value<string>()->default_value("0"),
            ...
            ("activate_webcam,w", bool_switch()->default_value(false),
             "Activate webcam to detect face landmarks? (true=1 or false=0)")
            ("record_all", bool_switch()->default_value(false),
...
variables_map parse_parameters(options_description options, int argc, const char* argv[]) {
...
bool are_parameters_ok(variables_map parameters_map, options_description options) {
...
Mission create_mission(variables_map parameters_map) {
    ...
    unsigned int frames_per_second = parameters_map["video_fps"].as<unsigned int>();
    unsigned int time_limit_in_seconds = parameters_map["time_limit"].as<unsigned int>();
    unsigned  int self_report_prompt_time_in_seconds = parameters_map["self_report"].as<unsigned int>();
