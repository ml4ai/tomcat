...
our intervention
%%% (threat room and victim type communication)
%%% \ps{We have more interventions. Moreover, we don't intervene on threat marker blocks anymore.}, 
%%% I have edited the sentence a bit. We do not need to specifically address all
%%% our interventions. 
or (iii)
...
\begin{itemize}
    ...
    \item \textbf{Rutgers Threat Room Communication AC}: \% of threat
        ...
        %
        \ps{Maybe by increasing coordination, the number of overall markers will increase too.     However, we do not encourage the players to talk about threat markers anymore. The agent only intervenes on markers that instill assistance from another player.}  
        %
        \kobus{We are not declaring the details of what we expect. These are not
        our measures. We are simply testing whether they are indicative of
        coordination.}
...
%
    ...
    It is also possible that there are multiple coordinating processes. However,
    for coordination processes that we typically would like to study, the
    observations of different modalities and time scales are all linked to the
    latent coordination obeying a single temporal assumption. 

    To pre-register a single approach, we will assume that coordination drifts
    slowly during the first half of each trial, and is constant for the second
    half of each trial. This allows for the team to develop some rapport and
    for interventions to have an effect. 
