...
to perform beyond the sum of their parts\citep{roberts2022state}\ap{Add a space
before each \texttt{\\citep} command}. In particular, 
...
Currently, most real-time natural language understanding(NLU)\ap{Add a space
before each opening parenthesis.} and responding systems(e.g. Question and
answering\citep{hawkins2017you}\ap{The Google Scholar BibTeX entry is a bit
    lacking - the bibliography entry seems incomplete. Can you get the BibTeX
    entry from the corresponding PsyArXiv paper instead? The instructions here
might help you: \url{https://adarsh.cc/posts/citations.html}}, incremental
...
literature\ap{Add citations for the papers with the different definitions of
CLC.}, for our module we adopt the definition proposed by
...
%\autoref{tab:clc-triggers-labels}, and an algorithm for CLC detection is shown
in Algorithm 1.
