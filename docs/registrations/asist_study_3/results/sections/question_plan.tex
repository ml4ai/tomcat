...
(ToM),\ap{This sounds like we are using ToM as a tool for this analysis. Is
that what you mean? If yes, how are we using ToM?}
...
assumption, declare it explicitly as such.} represented by a
...
\begin{enumerate}
    ...
    \item Can people listen to spoken questions and accurately decide if the
        other person's plan is simple, sequential, or hierarchical?\ap{Are not
        ...
        The connection is not obvious, you should clarify.}?
\end{enumerate}

%%%%%

Previous research \ap{Cite references} into question-\emph{answering, not asking,} centered on
optimization \ap{Optimization of what?} because researchers assumed that people
prefer concise questions and answers. However, people do not engage in dialog
...
build rapport with each other. The current literature regarding ToM and
question-answering is sparse but even more so for question \emph{asking}. 
