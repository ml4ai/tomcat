...
def estimate_g_from_gibbs_sampling(pgm):
    ...
    observations = pd.DataFrame([{('s', 0):0, ('l', 0):1}])
    samples, _ = sampling.sample(1000, 100, observations)
    # df = pd.DataFrame([[state[1] for _, state in sample.items()] for sample in samples],
    #                   columns=[node[0] for node, _ in samples[0].items()])
    print(samples[('pi_d,0')].value_counts(normalize=True))
