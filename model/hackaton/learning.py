...
class ParameterLearning:
    ...
    def estimate_parameters(self, evidence_set, number_of_samples, burn_in, show_progress=True):
        """
        ...
        theta_s_samples = np.zeros((number_of_samples, self.model.number_of_states, self.model.number_of_states))
        pi_lt_samples = np.zeros((number_of_samples, self.model.number_of_states, 2))

        # Start by sampling State_t using simple ancestral sampling
        states_sample = self.get_initial_states_sample_from_parameters_priors(evidence_set.number_of_data_points,
                                                                              evidence_set.time_slices)
        for _ in tqdm(range(burn_in), desc='Burn-in', disable=not show_progress):
            local_model.cpd_tables, states_sample = self.sample(evidence_set, local_model.cpd_tables, states_sample)
    ...
    def get_initial_states_sample_from_parameters_priors(self, number_of_data_points, time_slices):
        """
        ...
        theta_s_sampled = np.zeros((self.model.number_of_states, self.model.number_of_states))
        for state in range(self.model.number_of_states):
            theta_s_sampled[state] = dirichlet(self.model.parameter_priors.theta_s_priors[state]).rvs()[0]
    ...
    def sample_states_over_time(self, theta_s, time_slices):
        ...
        for t in range(1, time_slices + 1):
            states_sample[t] = np.random.choice(self.model.number_of_states, p=theta_s[states_sample[t - 1]])
    ...
    def sample(self, evidence_set, cpd_tables, states_sample):
        ...
        cpd_tables.theta_s, cpd_tables.pi_lt = self.sample_parameters(evidence_set, states_sample)
        states_sample = self.sample_states(evidence_set, cpd_tables, states_sample)
    ...
    def sample_parameters(self, evidence_set, state_sample):
        ...
        # Sample parameters
        theta_s_sample = np.zeros((self.model.number_of_states, self.model.number_of_states))
    ...
    def sample_states(self, evidence_set, distributions, states_sample):
        ...
        """
        posterior_state = np.zeros((evidence_set.number_of_data_points, self.model.number_of_states))

        for t in range(1, evidence_set.time_slices):
            posterior_state += utils.log(distributions.theta_s[states_sample[:, t - 1]]) + utils.log(
                distributions.pi_lt[:, evidence_set.lt_evidence[:, t]]).T + utils.log(
                distributions.theta_rm[:, evidence_set.rm_evidence[:, t]]).T + utils.log(
                distributions.pi_tg[:, evidence_set.tg_evidence[:, t]]).T + utils.log(
            ...
            for d, posterior in enumerate(posterior_state):
                states_sample[d, t] = np.random.choice(self.model.number_of_states, p=posterior)
