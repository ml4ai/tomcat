...
class ModelInference():
    ...
    def get_triaging_marginals_over_time(self, evidence_set):
        ...
        tg_marginals = np.zeros(
            (evidence_set.number_of_data_points, evidence_set.time_slices - 1, self.model.cpd_tables.pi_tg.shape[1]))
        ty_marginals = np.zeros(
            (evidence_set.number_of_data_points, evidence_set.time_slices - 1, self.model.cpd_tables.pi_ty.shape[1]))

        # The last matrix that shows up in the formula is the same, and it's being precomputed here for efficiency
        tg_last_term = np.stack([np.sum(self.model.cpd_tables.theta_s * self.model.cpd_tables.pi_tg[:, col], axis=1) for col in
                                 range(self.model.cpd_tables.pi_tg.shape[1])])
        ty_last_term = np.stack([np.sum(self.model.cpd_tables.theta_s * self.model.cpd_tables.pi_ty[:, col], axis=1) for col in
                                 range(self.model.cpd_tables.pi_ty.shape[1])])
        ...
        for d in tqdm(range(evidence_set.number_of_data_points), 'Computing marginals'):
            ...
            partials = self.get_partials(lt_evidence, rm_evidence, tg_evidence, ty_evidence)
            ...
            tg_marginals[d, 0:2, :] = self.get_two_first_marginals(partials, self.model.cpd_tables.theta_s, self.model.cpd_tables.pi_tg,
                                                                   tg_last_term)
            ty_marginals[d, 0:2, :] = self.get_two_first_marginals(partials, self.model.cpd_tables.theta_s, self.model.cpd_tables.pi_ty,
                                                                   ty_last_term)
            ...
            for t in range(2, evidence_set.time_slices - 1):
                ...
                for j in range(t - 1, 0, -1):
                    tg_tmp = np.stack(
                        [np.sum(partials[j] * tg_tmp[row], axis=1) for row in range(self.model.cpd_tables.pi_tg.shape[1])])
                    tg_tmp /= np.sum(tg_tmp, axis=0)

                    ty_tmp = np.stack(
                        [np.sum(partials[j] * ty_tmp[row], axis=1) for row in range(self.model.cpd_tables.pi_tg.shape[1])])
    ...
    def get_partials(self, lt_evidence, rm_evidence, tg_evidence, ty_evidence):
        ...
        partial = utils.log(self.model.cpd_tables.theta_s[0]) + utils.log(lt) + utils.log(rm) + utils.log(tg) + utils.log(ty)
        ...
        for t in range(2, time_slices):
            ...
            partial = utils.log(self.model.cpd_tables.theta_s) + utils.log(lt) + utils.log(rm) + utils.log(tg) + utils.log(ty)
