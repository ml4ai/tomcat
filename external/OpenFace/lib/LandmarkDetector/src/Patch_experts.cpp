...
std::vector<int> Patch_experts::Collect_visible_landmarks(
    vector<vector<cv::Mat_<int>>> visibilities, int scale, int view_id, int n) {
    std::vector<int> vis_lmk;
    for (int i = 0; i < n; i++) {
        if (visibilities[scale][view_id].rows == n) {
            if (visibilities[scale][view_id].at<int>(i, 0) != 0) {
                // For CEN patch experts and frontal views skip the mirror
                // indices
                if (!cen_expert_intensity.empty()) {

                    // If frontal view we can do mirrored landmarks together
                    if (view_id == 0) {
                        // If the patch expert does not have values, means it's
                        // a mirrored version and will be done in another part
                        // of a loop
                        if (!cen_expert_intensity[scale][view_id][i]
                                 .biases.empty()) {
                            vis_lmk.push_back(i);
                        }
                    }
...
void Patch_experts::Response(vector<cv::Mat_<float>>& patch_expert_responses,
    ...
    // similarity and inverse similarity transform to and from image and
    // reference shape
    ...
    if (use_ccnf) {
        ...
        for (size_t w_size = 0; w_size < this->sigma_components.size();
             ++w_size) {
    ...
    if (use_cen) {
        ...
        interpolationMatrix(interp_mat,
                            resp_size,
                            resp_size,
    ...
    // calculate the patch responses for every landmark (this is the heavy
    // lifting of landmark detection)
    parallel_for_(cv::Range(0, vis_lmk.size()), [&](const cv::Range& range) {
        for (int i = range.start; i < range.end; i++) {

            // Work out how big the area of interest has to be to get a response
            // of window size
            ...
            if (use_cen) {
                ...
                area_of_interest_height =
                    window_size +
                    cen_expert_intensity[scale][view_id][ind].height_support -
                    1;
            }
            else if (use_ccnf) {
                area_of_interest_width =
                    window_size +
                    ccnf_expert_intensity[scale][view_id][ind].width - 1;
                area_of_interest_height =
                    window_size +
                    ccnf_expert_intensity[scale][view_id][ind].height - 1;
            }
            else {
                area_of_interest_width =
                    window_size +
                    svr_expert_intensity[scale][view_id][ind].width - 1;
                area_of_interest_height =
                    window_size +
                    svr_expert_intensity[scale][view_id][ind].height - 1;
            }

            // scale and rotate to mean shape to reference frame
            cv::Mat sim = (cv::Mat_<float>(2, 3) << a1,
                           -b1,
            ...
            // location
            cv::Mat_<float> area_of_interest(
                area_of_interest_height, area_of_interest_width, 0.0f);

            cv::warpAffine(grayscale_image,
                           area_of_interest,
                           sim,
                           area_of_interest.size(),
                           cv::WARP_INVERSE_MAP + cv::INTER_LINEAR);

            // Get intensity response either from the SVR, CCNF, or CEN patch
            // experts (prefer CEN as they are the most accurate so far)
            if (!cen_expert_intensity.empty()) {

                int im2col_size =
                    (area_of_interest_width * area_of_interest_height - 1) / 2;

                cv::Mat_<float> prealloc_mat =
                    preallocated_im2col[ind][im2col_size];

                // If frontal view we can do mirrored landmarks together
                if (view_id == 0) {
                    // If the patch expert does not have values, means it's a
                    // mirrored version and will be done in another part of a
                    // loop
                    if (!cen_expert_intensity[scale][view_id][ind]
                             .biases.empty()) {
                        // No mirrored expert, so do normally
                        int mirror_id = mirror_inds.at<int>(ind);
                        if (mirror_id == ind) {
                            cv::Mat_<float> empty(0, 0, 0.0f);
                            cen_expert_intensity[scale][view_id][ind]
                                .ResponseSparse(area_of_interest,
                                                empty,
                            ...
                            cv::Mat sim_r =
                                (cv::Mat_<float>(2, 3) << a1,
                                 ...
                                 landmark_locations.at<float>(mirror_id, 0) -
                                     a1 * (area_of_interest_width - 1.0f) /
                                         2.0f +
                                     b1 * (area_of_interest_width - 1.0f) /
                                         2.0f,
                                 b1,
                                 a1,
                                 landmark_locations.at<float>(mirror_id + n,
                                                              0) -
                                     a1 * (area_of_interest_width - 1.0f) /
                                         2.0f -
                                     b1 * (area_of_interest_width - 1.0f) /
                            ...
                            // Extract the region of interest around the current
                            // landmark location
                            cv::Mat_<float> area_of_interest_r(
                                area_of_interest_height,
                                area_of_interest_width,
                                0.0f);
                ...
                patch_expert_responses[ind] =
                    cv::Mat_<float>(window_size, window_size);
                ...
                // Below is an alternative way to compute the same, but that
                // uses FFT instead of OpenBLAS
...
//===========================================================================
bool Patch_experts::Read(vector<string> intensity_svr_expert_locations,
...
bool Patch_experts::Read_SVR_patch_experts(
    ...
    if (patchesFile.is_open()) {
        ...
        // the visibility of points for each of the views (which verts are
        // visible at a specific view
...
bool Patch_experts::Read_CCNF_patch_experts(
    ...
    if (patchesFile.is_open()) {
        ...
        // patch reading (this assumes all of them have the same type, and
        // number of betas)
        int num_win_sizes;
        int num_sigma_comp;
        ...
        for (int w = 0; w < num_win_sizes; ++w) {
            ...
            for (int s = 0; s < num_sigma_comp; ++s) {
                LandmarkDetector::ReadMatBin(patchesFile,
                                             sigma_components[w][s]);
            }
        }
...
bool Patch_experts::Read_CEN_patch_experts(
    ...
    if (patchesFile.is_open()) {
        ...
        cout << "Could not find CEN patch experts, for instructions of how to "
                "download them, see "
