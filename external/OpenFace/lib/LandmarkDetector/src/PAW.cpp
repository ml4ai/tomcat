...
PAW::PAW(const cv::Mat_<float>& destination_shape,
    ...
    for (int tri = 0; tri < num_tris; ++tri) {
        ...
        beta.at<float>(tri, 0) =
            (xs.at<float>(j) * c4 - ys.at<float>(j) * c3) / c5;
        ...
        cv::Vec3f xs_three(
            triangle_points[0], triangle_points[2], triangle_points[4]);
        cv::Vec3f ys_three(
            triangle_points[1], triangle_points[3], triangle_points[5]);
...
void PAW::Warp(const cv::Mat& image_to_warp,
        ...
        for (int x = 0; x < pixel_mask.cols; x++) {
            ...
            if (*mp == 0) {
                ...
                *yp = -1;
                ...
                // coefficients This will always be the case in the first
                // iteration, hence a will not point to nothing
                if (j != k) {
                    // Update the coefficient pointer if a new triangle is being
                    // processed
                ...
                // look at the first coefficient (and increment). first
                // coefficient is an x offset
                float xo = *ap++;
...
// triangle
bool PAW::pointInTriangle(float x0,
                          float y0,
                          float x1,
                          float y1,
                          float x2,
...
int PAW::findTriangle(const cv::Point_<float>& point,
    ...
    if (guess != -1) {
        ...
        bool in_triangle = pointInTriangle(x0,
                                           y0,
    ...
    for (int i = 0; i < num_tris; ++i) {
        ...
        // Skip the check if the point is outside the bounding box of the
        // triangle
