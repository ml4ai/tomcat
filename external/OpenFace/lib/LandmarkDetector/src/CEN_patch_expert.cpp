...
void CEN_patch_expert::ResponseInternal(cv::Mat_<float>& response) {
    for (size_t layer = 0; layer < activation_function.size(); ++layer) {
        ...
        // We are performing response = weights[layers] * response, but in
        // OpenBLAS as that is significantly quicker than OpenCV
        cv::Mat_<float> resp = response;
        float* m1 = (float*)resp.data;
        ...
        // Adding the bias (bit ugly, but the fastest way to do this), TODO can
        // this bias be incorporated in the above?
        float* data = (float*)response.data;
...
void CEN_patch_expert::ResponseSparse(
    ...
    if (right_provided) {
        ...
        im2colBiasSparseContrastNorm(area_of_interest_right,
                                     width_support,
                                     height_support,
                                     im2col_prealloc_right);
    }

    // Extract im2col but in a sparse way and contrast normalize
    if (left_provided) {
        response_height = area_of_interest_left.rows - height_support + 1;
        im2colBiasSparseContrastNorm(area_of_interest_left,
                                     width_support,
                                     height_support,
                                     im2col_prealloc_left);
    }
