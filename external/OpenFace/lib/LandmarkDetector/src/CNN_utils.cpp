...
namespace LandmarkDetector {
      ...
      for (int k = 0; k < prelu_weights.rows; ++k) {
        ...
        for (int i = 0; i < w; ++i) {
          ...
          *iter = in_val >= 0 ? in_val : in_val * neg_mult;
          ...
          // To deal with OpenCV 3.4s debug mode not allowing to go over
  ...
  convolution_fft2(std::vector<cv::Mat_<float>>& outputs,
    ...
    for (size_t k = 0; k < kernels.size(); ++k) {
      ...
      convolution_single_kern_fft(
          input_maps, input_image_dft, kernels[k], precomp_dfts[k], output);
  ...
  void convolution_direct_blas(std::vector<cv::Mat_<float>>& outputs,
    ...
    // data and re-allocate only if not enough rows are present, this is what
    // makes this non thread safe, as same memory would be used
