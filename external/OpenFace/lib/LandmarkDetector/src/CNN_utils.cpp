...
namespace LandmarkDetector {
            ...
            for (int k = 0; k < prelu_weights.rows; ++k) {
                ...
                for (int i = 0; i < w; ++i) {
                    ...
                    *iter = in_val >= 0 ? in_val : in_val * neg_mult;
                    ...
                    // To deal with OpenCV 3.4s debug mode not allowing to go
                    // over iteration boundaries
                    if (i + 1 < w) {
                        iter++;
                    }
                }
        ...
        if (input_maps.size() > 1) {
            ...
            if (input_concat.rows == weights.cols) {
                ...
                for (int k = 0; k < biases.rows; ++k) {
                    input_concat.row(k) =
                        input_concat.row(k) + biases.at<float>(k);
                }
                ...
                // Flatten the input
                input_concat = input_concat.reshape(
                    0, input_concat.rows * input_concat.cols);
    ...
    void max_pooling(std::vector<cv::Mat_<float>>& outputs,
        ...
        for (size_t in = 0; in < input_maps.size(); ++in) {
            ...
            int out_x =
                (int)round((float)(input_maps[in].cols - kernel_size_x) /
                           (float)stride_x) +
                1;
            int out_y =
                (int)round((float)(input_maps[in].rows - kernel_size_y) /
                           (float)stride_y) +
                1;
    ...
    void
    convolution_single_kern_fft(const vector<cv::Mat_<float>>& input_imgs,
                                vector<cv::Mat_<double>>& img_dfts,
                                const vector<cv::Mat_<float>>& _templs,
                                map<int, vector<cv::Mat_<double>>>& _templ_dfts,
        ...
        dftsize.width =
            cv::getOptimalDFTSize(result.cols + _templs[0].cols - 1);
        dftsize.height =
            cv::getOptimalDFTSize(result.rows + _templs[0].rows - 1);
        ...
        if (_templ_dfts.find(dftsize.width) == _templ_dfts.end()) {
            ...
            for (size_t k = 0; k < _templs.size(); ++k) {
                ...
                cv::Mat_<double> dst(
                    dftTempl[k], cv::Rect(0, 0, dftsize.width, dftsize.height));
    ...
    convolution_fft2(std::vector<cv::Mat_<float>>& outputs,
                     ...
                     vector<map<int, vector<cv::Mat_<double>>>>& precomp_dfts) {
        outputs.clear();

        // Useful precomputed data placeholders for quick correlation
        // (convolution)
        ...
        for (size_t k = 0; k < kernels.size(); ++k) {
            ...
            convolution_single_kern_fft(input_maps,
                                        input_image_dft,
                                        kernels[k],
                                        precomp_dfts[k],
                                        output);
    ...
    void im2col_t(const cv::Mat_<float>& input,
        ...
        // determine how many blocks there will be with a sliding window of
        // width x height in the input
    ...
    void im2col(const cv::Mat_<float>& input,
                const unsigned int width,
                const unsigned int height,
                cv::Mat_<float>& output) {
    ...
    void im2col_multimap(const vector<cv::Mat_<float>>& inputs,
                         const unsigned int width,
                         const unsigned int height,
                         cv::Mat_<float>& output) {
        ...
        if (output.cols != width * height * inputs.size() + 1 ||
            ...
            output =
                cv::Mat::ones(xB * yB, width * height * num_maps + 1, CV_32F);
        }
        ...
        for (unsigned int i = 0; i < yB; i++) {
            ...
            for (unsigned int j = 0; j < xB; j++) {
                ...
                // iterate over the blocks within the image
                for (unsigned int yy = 0; yy < height; ++yy) {
                    for (unsigned int in_maps = 0; in_maps < num_maps;
                         ++in_maps) {
                        // Faster iteration over the image
    ...
    void convolution_direct_blas(std::vector<cv::Mat_<float>>& outputs,
        ...
        // Instead of re-allocating data use the first rows of already allocated
        // data and re-allocate only if not enough rows are present, this is
        // what makes this non thread safe, as same memory would be used
        ...
        // cblas_sgemm(CblasColMajor, CblasNoTrans, CblasNoTrans, weight_t.cols,
        // yB
        // * xB, pre_alloc_im2col.cols, 1, m2, weight_t.cols, m1,
        // pre_alloc_im2col.cols, 0.0, m3, weight_t.cols);
        float alpha = 1.0f;
        float beta = 0.0f;
