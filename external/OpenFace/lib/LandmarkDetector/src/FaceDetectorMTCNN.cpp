...
CNN::CNN(const CNN& other)
    ...
    this->cnn_convolutional_layers.resize(
        other.cnn_convolutional_layers.size());
    for (size_t l = 0; l < other.cnn_convolutional_layers.size(); ++l) {
        ...
        for (size_t i = 0; i < other.cnn_convolutional_layers[l].size(); ++i) {
            ...
            for (size_t k = 0; k < other.cnn_convolutional_layers[l][i].size();
                 ++k) {
                // Make sure the matrix is copied.
    ...
    for (size_t l = 0; l < other.cnn_fully_connected_layers_weights.size();
         ++l) {
        // Make sure the matrix is copied.
    ...
    for (size_t l = 0; l < other.cnn_fully_connected_layers_biases.size();
         ++l) {
        // Make sure the matrix is copied.
    ...
    for (size_t l = 0; l < other.cnn_prelu_layer_weights.size(); ++l) {
        // Make sure the matrix is copied.
        this->cnn_prelu_layer_weights[l] =
            other.cnn_prelu_layer_weights[l].clone();
    }
}
...
CNN::Inference(const cv::Mat& input_img, bool direct, bool thread_safe) {
    ...
    for (size_t layer = 0; layer < cnn_layer_types.size(); ++layer) {
        ...
        if (layer_type == 0) {
            ...
            // Either perform direct convolution through matrix multiplication
            // or use an FFT optimized version, which one is optimal depends on
            // the kernel and input sizes
            if (direct) {
                if (thread_safe) {
            ...
            // convolution_fft(outs, input_maps,
            // cnn_convolutional_layers[cnn_layer],
            // cnn_convolutional_layers_bias[cnn_layer],
            // cnn_convolutional_layers_dft[cnn_layer]);
        ...
        if (layer_type == 1) {
            ...
            int kernel_size_x =
                std::get<0>(cnn_max_pooling_layers[max_pool_layer]);
            int kernel_size_y =
...
void CNN::ClearPrecomp() {
    for (size_t k1 = 0; k1 < cnn_convolutional_layers_dft.size(); ++k1) {
        for (size_t k2 = 0; k2 < cnn_convolutional_layers_dft[k1].size();
             ++k2) {
    ...
    if (cnn_stream.is_open()) {
        ...
        for (int layer = 0; layer < network_depth; ++layer) {
            ...
            if (layer_type == 0) {
                ...
                vector<map<int, vector<cv::Mat_<double>>>>
                ...
                // Rearrange the flattened kernels into weight matrices for
                ...
                cv::Mat_<float> W(
                    weight_matrix.rows, weight_matrix.cols + 1, 1.0);
...
void FaceDetectorMTCNN::Read(const string& location) {
    ...
    boost::filesystem::path root =
        boost::filesystem::path(location).parent_path();
...
non_maximum_supression(const std::vector<cv::Rect_<float>>& original_bb,
    ...
    // Sort the input bounding boxes by the detection score, using the nice
    // trick of multimap always being sorted internally
    ...
    while (idxs.size() > 0) {
        ...
        // Iterate through remaining bounding boxes and choose which ones to
        // remove
        for (auto pos = std::begin(idxs); pos != std::end(idxs);) {
            ...
            // Remove the bounding boxes with less confidence but with
            // significant overlap with the current one
            if (overlap > thresh) {
                pos = idxs.erase(pos);
            }
            else {
    ...
    for (int x = 0; x < heatmap.cols; ++x) {
        for (int y = 0; y < heatmap.rows; ++y) {
            if (heatmap.at<float>(y, x) >= threshold) {
                ...
                o_bounding_boxes.push_back(cv::Rect_<float>(
...
void apply_correction(vector<cv::Rect_<float>>& total_bboxes,
    ...
    for (size_t i = 0; i < total_bboxes.size(); ++i) {
        ...
        float new_max_y = curr_box.y + curr_box.height +
                          curr_box.height * corrections[i].height;
...
bool FaceDetectorMTCNN::DetectFaces(vector<cv::Rect_<float>>& o_regions,
    ...
    vector<vector<cv::Rect_<float>>> proposal_corrections_cross_scale(
        num_scales);
    ...
    for (int i = 0; i < num_scales; ++i) {
        ...
        generate_bounding_boxes(proposal_boxes,
                                scores,
    ...
    // Perform non-maximum supression on proposals accross scales and combine
    // them
    for (int i = 0; i < num_scales; ++i) {
        vector<int> to_keep = non_maximum_supression(
            proposal_boxes_cross_scale[i], scores_cross_scale[i], 0.5, false);
        select_subset(to_keep,
                      proposal_boxes_cross_scale[i],
                      scores_cross_scale[i],
        ...
        scores_all.insert(scores_all.end(),
                          scores_cross_scale[i].begin(),
    ...
    select_subset(
        to_keep, proposal_boxes_all, scores_all, proposal_corrections_all);
    ...
    for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
        ...
        int end_x_in = cv::min(
            (int)(proposal_boxes_all[k].x + width_target - 1), width_orig);
        int end_y_in = cv::min(
            (int)(proposal_boxes_all[k].y + height_target - 1), height_orig);
        ...
        int end_x_out =
            cv::min(width_target - (proposal_boxes_all[k].x +
                    ...
                    width_target);
        int end_y_out = cv::min(height_target - (proposal_boxes_all[k].y +
                                                 proposal_boxes_all[k].height -
                                                 height_orig),
                                height_target);

        cv::Mat tmp(height_target,
                    width_target,
                    CV_32FC3,
                    cv::Scalar(0.0f, 0.0f, 0.0f));

        img_float(cv::Rect(start_x_in,
                           start_y_in,
                           end_x_in - start_x_in,
                           end_y_in - start_y_in))
            .copyTo(tmp(cv::Rect(start_x_out,
                                 start_y_out,
                                 end_x_out - start_x_out,
        ...
        float prob = 1.0 / (1.0 + cv::exp(rnet_out[0].at<float>(0) -
                                          rnet_out[0].at<float>(1)));
        scores_all[k] = prob;
    ...
    for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
        ...
        float prob = 1.0 / (1.0 + cv::exp(onet_out[0].at<float>(0) -
                                          onet_out[0].at<float>(1)));
        scores_all[k] = prob;
    ...
    for (size_t k = 0; k < proposal_boxes_all.size(); ++k) {
        ...
        o_regions.push_back(cv::Rect_<float>(proposal_boxes_all[k].x,
                                             proposal_boxes_all[k].y,
                                             proposal_boxes_all[k].width,
                                             proposal_boxes_all[k].height));
