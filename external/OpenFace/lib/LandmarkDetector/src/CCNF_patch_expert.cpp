...
void CCNF_patch_expert::ComputeSigmas(
    ...
    // Each of the landmarks will have the same connections, hence constant
    // number of sigma components
...
//===========================================================================
void CCNF_neuron::Response(const cv::Mat_<float>& im,
    ...
    if (neuron_type == 3) {
        ...
        matchTemplate_m(I,
                        ...
                        weights_dfts,
                        resp,
                        cv::TM_CCOEFF); // the linear multiplication, efficient
                                        // calc of response
    }
    else {
        matchTemplate_m(I,
...
void CCNF_patch_expert::Read(
    ...
    for (size_t i = 0; i < neurons.size(); i++) {
        ...
        weights_flat.copyTo(weight_matrix(cv::Rect(
            1, i, neurons[i].weights.rows * neurons[i].weights.cols, 1)));
...
void CCNF_patch_expert::Response(const cv::Mat_<float>& area_of_interest,
    ...
    for (size_t i = 0; i < neurons.size(); i++) {
        ...
        if (neurons[i].alpha > 1e-4) {
            ...
            neurons[i].Response(area_of_interest,
...
void CCNF_patch_expert::ResponseOpenBlas(
    ...
    im2colContrastNormBias(area_of_interest,
                           neurons[0].weights.cols,
                           neurons[0].weights.rows,
                           im2col_prealloc);
    ...
    cv::Mat_<float> neuron_resp_full(
        weight_matrix.rows, normalized_input.cols, 0.0f);
    ...
    // cv::Mat_<float> neuron_resp_full = this->weight_matrix *
    // normalized_input;
    ...
    for (size_t i = 0; i < neurons.size(); i++) {
        if (neurons[i].alpha > 1e-4) {
            ...
            cv::MatIterator_<float> q1 =
                rel_row.begin(); // respone for each pixel
