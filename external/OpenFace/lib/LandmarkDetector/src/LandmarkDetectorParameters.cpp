...
namespace LandmarkDetector {
    ...
    FaceModelParameters::FaceModelParameters(vector<string>& arguments) {
        ...
        // First element is reserved for the executable location (useful for
        // finding relative model locs)
    ...
    void FaceModelParameters::init() {
        ...
        // Off by default (as it might lead to some slight inaccuracies in
        // slowly moving faces)
        ...
        if (!getenv("OPENFACE_MODELS_DIR")) {
            throw runtime_error(
                "The OPENFACE_MODELS_DIR environment variable is not set - it "
                "needs to be set to point to the directory containing the "
                "OpenFace models. Exiting now.");
        }

        this->OpenFace_models_dir =
            path(getenv("OPENFACE_MODELS_DIR")).string();
        ...
        this->weight_factor = 0.0f; // By default do not use NU-RLMS for videos
                                    // as it does not work as well for them
