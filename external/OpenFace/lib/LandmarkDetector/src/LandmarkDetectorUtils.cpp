...
namespace LandmarkDetector {
    ...
    void crossCorr_m(const cv::Mat_<float>& img,
                     cv::Mat_<double>& img_dft,
                     const cv::Mat_<float>& _templ,
                     map<int, cv::Mat_<double>>& _templ_dfts,
                     cv::Mat_<float>& corr) {
        ...
        if (_templ_dfts.find(dftsize.width) == _templ_dfts.end()) {
            ...
            cv::Mat_<double> dst1(dftTempl,
                                  cv::Rect(0, 0, _templ.cols, _templ.rows));

            if (dst1.data != src.data)
                src.convertTo(dst1, dst1.depth());

            if (dst.cols > _templ.cols) {
                cv::Mat_<double> part(dst,
                                      cv::Range(0, _templ.rows),
        ...
        if (img_dft.empty()) {
            ...
            cv::Size dsz(bsz.width + _templ.cols - 1,
                         bsz.height + _templ.rows - 1);
    ...
    void matchTemplate_m(const cv::Mat_<float>& input_img,
        ...
        int numType =
            method == cv::TM_CCORR || method == cv::TM_CCORR_NORMED
                ? 0
        ...
        LandmarkDetector::crossCorr_m(
            input_img, img_dft, templ, templ_dfts, result);
    ...
    // Useful utility for grabing a bounding box around a set of 2D landmarks
    // (as a 1D 2n x 1 vector of xs followed by doubles or as an n x 2 vector)
    ...
    CalculateVisibleLandmarks(const cv::Mat_<float>& shape2D,
        ...
        for (int i = 0; i < n; ++i) {
            if (visibilities.at<int>(i)) {
                cv::Point2f featurePoint(shape2D.at<float>(i),
                                         shape2D.at<float>(i + n));
    ...
    vector<cv::Point2f> CalculateAllLandmarks(const cv::Mat_<float>& shape2D) {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            if (shape2D.cols == 1) {
                ...
                featurePoint = cv::Point2f(shape2D.at<float>(i, 0),
                                           shape2D.at<float>(i, 1));
            }
    ...
    vector<cv::Point2f> CalculateVisibleLandmarks(const CLNF& clnf_model) {
        ...
        if (clnf_model.detection_success) {
            int idx = clnf_model.patch_experts.GetViewIdx(
                clnf_model.params_global, 0);
            // Because we only draw visible points, need to find which points
            // patch experts consider visible at a certain orientation
    ...
    vector<cv::Point2f> CalculateVisibleEyeLandmarks(const CLNF& clnf_model) {
        ...
        for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
            ...
            if (clnf_model.hierarchical_model_names[i].compare("left_eye_28") ==
                    0 ||
                clnf_model.hierarchical_model_names[i].compare(
                    "right_eye_28") == 0) {

                auto lmks = CalculateVisibleLandmarks(
                    clnf_model.hierarchical_models[i]);
                for (auto lmk : lmks) {
    ...
    vector<cv::Point3f> Calculate3DEyeLandmarks(
        ...
        for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
            ...
            if (clnf_model.hierarchical_model_names[i].compare("left_eye_28") ==
                ...
                auto lmks =
                    clnf_model.hierarchical_models[i].GetShape(fx, fy, cx, cy);

                int num_landmarks = lmks.cols;

                for (int lmk = 0; lmk < num_landmarks; ++lmk) {
                    cv::Point3f curr_lmk(lmks.at<float>(0, lmk),
                                         lmks.at<float>(1, lmk),
                                         lmks.at<float>(2, lmk));
    ...
    vector<cv::Point2f> CalculateAllEyeLandmarks(const CLNF& clnf_model) {
        ...
        for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
            ...
            if (clnf_model.hierarchical_model_names[i].compare("left_eye_28") ==
                ...
                auto lmks =
                    CalculateAllLandmarks(clnf_model.hierarchical_models[i]);
                for (auto lmk : lmks) {
                    to_return.push_back(lmk);
                }
            }
    ...
    //============================================================================
    bool DetectFaces(vector<cv::Rect_<float>>& o_regions,
        ...
        if (classifier.empty()) {
            ...
            return DetectFaces(
                o_regions, intensity, classifier, min_width, roi);
        }
    }
    ...
    bool DetectFaces(vector<cv::Rect_<float>>& o_regions,
        ...
        if (min_width == -1) {
            classifier.detectMultiScale(
                intensity, face_detections, 1.2, 2, 0, cv::Size(50, 50));
        }
        else {
            classifier.detectMultiScale(intensity,
                                        face_detections,
                                        1.2,
                                        2,
                                        0,
        ...
        for (size_t face = 0; face < face_detections.size(); ++face) {
            ...
            // CLNF detector expects the bounding box to encompass from eyebrow
            // to chin in y, and from cheeck outline to cheeck outline in x, so
            // we need to compensate

            // The scalings were learned using the Face Detections on LFPW,
            // Helen, AFW and iBUG datasets, using ground truth and detections
            // from openCV
            ...
            // Move the face slightly to the right (as the width was made
            // smaller)
            region.x =
                face_detections[face].x + 0.0578f * face_detections[face].width;
            // Shift face down as OpenCV Haar Cascade detects the forehead as
            // well, and we're not interested
            region.y = face_detections[face].y +
                       face_detections[face].height * 0.2166f;
        ...
        if (detect_success) {
            ...
            if (face_detections.size() > 1) {
                ...
                for (size_t i = 0; i < face_detections.size(); ++i) {
                    ...
                    if (use_preferred) {
                        dist = sqrt(
                            (preference.x) * (face_detections[i].width / 2 +
                                              face_detections[i].x) +
                            (preference.y) * (face_detections[i].height / 2 +
                                              face_detections[i].y));
                        better = dist < best;
                    }
                    else {
                    ...
                    // Pick a closest face to preffered point or the biggest
                    // face
                    if (i == 0 || better) {
                        bestIndex = i;
                        best = dist;
    ...
    bool DetectFacesHOG(vector<cv::Rect_<float>>& o_regions,
                        const cv::Mat_<uchar>& intensity,
        ...
        return DetectFacesHOG(
            o_regions, intensity, detector, confidences, min_width, roi);
    }
    ...
    bool DetectFacesHOG(vector<cv::Rect_<float>>& o_regions,
        ...
        // Convert from int bounding box do a double one with corrections
        for (size_t face = 0; face < face_detections.size(); ++face) {
            // CLNF expects the bounding box to encompass from eyebrow to chin
            // in y, and from cheeck outline to cheeck outline in x, so we need
            // to compensate
            ...
            // smaller)
            region.x =
                (face_detections[face].rect.get_rect().tl_corner().x() +
                 0.0389f * face_detections[face].rect.get_rect().width()) /
                scaling;
            // Shift face down as OpenCV Haar Cascade detects the forehead as
            // well, and we're not interested
            region.y =
                (face_detections[face].rect.get_rect().tl_corner().y() +
                 0.1278f * face_detections[face].rect.get_rect().height()) /
                scaling;

            // Correct for scale
            region.width =
                (face_detections[face].rect.get_rect().width() * 0.9611) /
                scaling;
            region.height =
                (face_detections[face].rect.get_rect().height() * 0.9388) /
                scaling;

            // The scalings were learned using the Face Detections on LFPW and
            // Helen using ground truth and detections from the HOG detector
            if (min_width != -1) {
                if (region.width < min_width ||
                    region.x < ((float)intensity.cols) * roi.x ||
                    region.y < ((float)intensity.cols) * roi.y ||
    ...
    bool DetectSingleFaceHOG(cv::Rect_<float>& o_region,
        ...
        bool detect_success = LandmarkDetector::DetectFacesHOG(face_detections,
                                                               intensity_img,
                                                               detector,
                                                               confidences,
                                                               min_width,
        ...
        if (detect_success) {
            ...
            // keep the most confident one or the one closest to preference
            // point if set
            float best_so_far;
            if (use_preferred) {
                best_so_far = sqrt(
                    (preference.x -
                     (face_detections[0].width / 2 + face_detections[0].x)) *
                        (preference.x - (face_detections[0].width / 2 +
                                         face_detections[0].x)) +
                    (preference.y -
                     (face_detections[0].height / 2 + face_detections[0].y)) *
                        (preference.y - (face_detections[0].height / 2 +
                                         face_detections[0].y)));
            }
            else if (use_size) {
                best_so_far =
                    (face_detections[0].width + face_detections[0].height) /
                    2.0;
            }
            else {
            ...
            for (size_t i = 1; i < face_detections.size(); ++i) {
                ...
                if (use_preferred) {
                    ...
                    dist =
                        (face_detections[i].width + face_detections[i].height) /
                        2.0;
                    better = dist > best_so_far;
                }
                else {
            ...
            // if not detected
            o_region = cv::Rect_<float>(0, 0, 0, 0);
            // A completely unreliable detection (shouldn't really matter what
            // is returned here)
            confidence = -2;
        }
        return detect_success;
    ...
    bool DetectFacesMTCNN(vector<cv::Rect_<float>>& o_regions,
                          const cv::Mat& image,
