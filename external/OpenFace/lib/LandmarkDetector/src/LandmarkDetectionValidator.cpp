...
DetectionValidator::DetectionValidator(const DetectionValidator& other)
    ...
    for (size_t v = 0; v < other.cnn_convolutional_layers.size(); ++v) {
        ...
        for (size_t l = 0; l < other.cnn_convolutional_layers[v].size(); ++l) {
            ...
            for (size_t i = 0; i < other.cnn_convolutional_layers[v][l].size();
                 ++i) {
                this->cnn_convolutional_layers[v][l][i].resize(
                    other.cnn_convolutional_layers[v][l][i].size());

                for (size_t k = 0;
                     k < other.cnn_convolutional_layers[v][l][i].size();
                     ++k) {
    ...
    for (size_t v = 0; v < other.cnn_fully_connected_layers_weights.size();
         ++v) {
        this->cnn_fully_connected_layers_weights[v].resize(
            other.cnn_fully_connected_layers_weights[v].size());

        for (size_t l = 0;
             l < other.cnn_fully_connected_layers_weights[v].size();
             ++l) {
            // Make sure the matrix is copied.
    ...
    for (size_t v = 0; v < other.cnn_fully_connected_layers_biases.size();
         ++v) {
        this->cnn_fully_connected_layers_biases[v].resize(
            other.cnn_fully_connected_layers_biases[v].size());

        for (size_t l = 0;
             l < other.cnn_fully_connected_layers_biases[v].size();
             ++l) {
            // Make sure the matrix is copied.
...
void DetectionValidator::Read(string location) {
    ...
    if (detection_validator_stream.is_open()) {
        ...
        if (validator_type != 3) {
            cout << "ERROR: Using old face validator, no longer supported"
                 << endl;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            LandmarkDetector::ReadMatBin(detection_validator_stream,
                                         orientation_tmp);
            ...
            if (validator_type == 3) {
                ...
                for (int layer = 0; layer < network_depth; ++layer) {
                    ...
                    if (layer_type == 0) {
                        ...
                        // Rearrange the flattened kernels into weight matrices
                        // for direct convolution computation
                        cv::Mat_<float> weight_matrix(
                            num_in_maps * kernels_rearr[0][0].rows *
                                kernels_rearr[0][0].cols,
                            num_kernels);
                        for (int k = 0; k < num_kernels; ++k) {
                            for (int i = 0; i < num_in_maps; ++i) {
                                // Flatten the kernel
                                cv::Mat_<float> k_flat =
                                    kernels_rearr[k][i].t();
                        ...
                        // Transpose the weight matrix for more convenient
                        // computation
                        ...
                        weight_matrix.copyTo(W(cv::Rect(
                            0, 0, weight_matrix.cols, weight_matrix.rows)));
                        ...
                        cnn_fully_connected_layers_weights[i].push_back(
                            weights);
                    }
                }
            }
...
float DetectionValidator::Check(const cv::Vec3d& orientation,
    ...
    ExtractBoundingBox(
        detected_landmarks_local, min_x_f, max_x_f, min_y_f, max_y_f);

    cv::Mat_<float> xs = detected_landmarks_local(
        cv::Rect(0, 0, 1, detected_landmarks.rows / 2));
    cv::Mat_<float> ys = detected_landmarks_local(cv::Rect(
        0, detected_landmarks.rows / 2, 1, detected_landmarks.rows / 2));
    ...
    for (size_t layer = 0; layer < cnn_layer_types[view_id].size(); ++layer) {
        ...
        if (layer_type == 2) {
            ...
            fully_connected(
                outputs,
                input_maps,
                cnn_fully_connected_layers_weights[view_id]
                                                  [fully_connected_layer]
                                                      .t(),
                cnn_fully_connected_layers_biases[view_id]
                                                 [fully_connected_layer]);
            fully_connected_layer++;
        }
        if (layer_type == 3) // ReLU
        {
            ...
            for (size_t k = 0; k < input_maps.size(); ++k) {
                // Apply the ReLU
                cv::threshold(
                    input_maps[k], input_maps[k], 0, 0, cv::THRESH_TOZERO);
