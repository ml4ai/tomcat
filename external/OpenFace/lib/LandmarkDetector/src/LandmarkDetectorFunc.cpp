...
cv::Vec6f LandmarkDetector::GetPose(
    ...
    if (!clnf_model.detected_landmarks.empty() &&
        ...
        // params_global is a vector with the parameters describing the rigid shape [scale, euler_x, euler_y, euler_z, tx, ty]
        ...
        cv::Vec3f vec_rot(clnf_model.params_global[1],
...
cv::Vec6f LandmarkDetector::GetPoseWRTCamera(
    const CLNF& clnf_model, float fx, float fy, float cx, float cy) {
    if (!clnf_model.detected_landmarks.empty() &&
        clnf_model.params_global[0] != 0) {
        ...
        // Here we correct for the camera orientation, for this need to
        // determine the angle the camera makes with the head pose
        float z_x =
        ...
        cv::Matx33f head_rotation =
            Utilities::AxisAngle2RotationMatrix(vec_rot);
...
void UpdateTemplate(const cv::Mat_<uchar>& grayscale_image, CLNF& clnf_model) {
    ...
    clnf_model.pdm.CalcBoundingBox(
        bounding_box, clnf_model.params_global, clnf_model.params_local);

    // Make sure the box is not out of bounds
    cv::Rect_<int> bbox_tmp((int)bounding_box.x,
                            (int)bounding_box.y,
...
void CorrectGlobalParametersVideo(const cv::Mat_<uchar>& grayscale_image,
    ...
    clnf_model.pdm.CalcBoundingBox(
        init_box, clnf_model.params_global, clnf_model.params_local);
    ...
    cv::Mat_<uchar> image;
    if (scaling < 1) {
        cv::resize(clnf_model.face_template,
                   clnf_model.face_template,
                   cv::Size(),
                   scaling,
                   scaling);
    ...
    cv::Mat corr_out;
    cv::matchTemplate(
        image, clnf_model.face_template, corr_out, cv::TM_CCOEFF_NORMED);
...
bool LandmarkDetector::DetectLandmarksInVideo(const cv::Mat& rgb_image,
    ...
    if (clnf_model.tracking_initialised) {
        ...
        // The area of interest search size will depend if the previous track
        // was successful
    ...
    if ((!clnf_model.tracking_initialised &&
        ...
        // If the face detector has not been initialised and we're using it,
        // then read it in
        if (clnf_model.face_detector_HAAR.empty() &&
            params.curr_face_detector == params.HAAR_DETECTOR) {
            clnf_model.face_detector_HAAR.load(
        ...
        if (clnf_model.face_detector_MTCNN.empty() &&
            ...
            clnf_model.face_detector_MTCNN.Read(
        ...
        if (clnf_model.preference_det.x != -1 &&
            ...
            preference_det.x =
                clnf_model.preference_det.x * grayscale_image.cols;
            preference_det.y =
                clnf_model.preference_det.y * grayscale_image.rows;
            clnf_model.preference_det = cv::Point(-1, -1);
        }

        bool face_detection_success;
        if (params.curr_face_detector ==
            ...
            face_detection_success = LandmarkDetector::DetectSingleFaceMTCNN(
                bounding_box,
                rgb_image,
                clnf_model.face_detector_MTCNN,
                confidence,
                preference_det);
        }

        // Attempt to detect landmarks using the detected face (if unseccessful
        ...
        if (face_detection_success) {
            ...
            clnf_model.pdm.CalcParams(clnf_model.params_global,
                                      bounding_box,
            ...
            // TODO rem (should the multi-hyp version be only for CEN and not
            // CLNF?), otherwise poss too slow, and poss not accurate bool
            // landmark_detection_success =
            // clnf_model.DetectLandmarks(grayscale_image, params);

            // Do the actual landmark detection (and keep it only if successful)
            // Perform multi-hypothesis detection here (as face detector can
            // pick up multiple of them)
            ...
            // estimates if it's initial detection however, do not care if it
            // was successful as the validator might be wrong, so continue
            // trackig regardless
            if (!initial_detection && !landmark_detection_success) {
                ...
                clnf_model.landmark_likelihoods =
                    landmark_likelihoods_init.clone();
...
bool LandmarkDetector::DetectLandmarksInVideo(
    ...
    if (bounding_box.width > 0) {
        ...
        clnf_model.pdm.CalcParams(
            clnf_model.params_global, bounding_box, clnf_model.params_local);

        // indicate that face was detected so initialisation is not necessary
        clnf_model.tracking_initialised = true;
    }

    return DetectLandmarksInVideo(
        rgb_image, clnf_model, params, grayscale_image);
}
...
bool DetectLandmarksInImageMultiHypBasic(const cv::Mat_<uchar>& grayscale_image,
    ...
    vector<cv::Vec6f> best_global_parameters_h(
        clnf_model.hierarchical_models.size());
    vector<cv::Mat_<float>> best_local_parameters_h(
        clnf_model.hierarchical_models.size());
    vector<cv::Mat_<float>> best_detected_landmarks_h(
        clnf_model.hierarchical_models.size());
    vector<cv::Mat_<float>> best_landmark_likelihoods_h(
        clnf_model.hierarchical_models.size());
        ...
        if (hypothesis == 0 || best_likelihood < clnf_model.model_likelihood) {
            ...
            for (size_t part = 0; part < clnf_model.hierarchical_models.size();
                ...
                best_detected_landmarks_h[part] =
                    clnf_model.hierarchical_models[part]
                        .detected_landmarks.clone();
                best_landmark_likelihoods_h[part] =
                    clnf_model.hierarchical_models[part]
                        .landmark_likelihoods.clone();
            }
        }
    }
        ...
        float lhood =
            clnf_model.model_likelihood *
                clnf_model.patch_experts
                    .early_term_weights[clnf_model.view_used] +
    ...
    if (!early_term) {
        ...
        for (size_t i = 0; i < max; ++i) {
            ...
            if (i == 0 || best_likelihood < clnf_model.model_likelihood) {
                ...
                best_local_parameters = clnf_model.params_local.clone();
                best_detected_landmarks = clnf_model.detected_landmarks.clone();
                best_landmark_likelihoods =
                    clnf_model.landmark_likelihoods.clone();
                best_success = success;

                for (size_t part = 0;
                     part < clnf_model.hierarchical_models.size();
                     ++part) {
                    best_likelihood_h[part] =
                        clnf_model.hierarchical_models[part].model_likelihood;
                    best_global_parameters_h[part] =
...
bool LandmarkDetector::DetectLandmarksInImage(
    ...
    if (clnf_model.patch_experts.early_term_biases.size() == 0) {
        success = DetectLandmarksInImageMultiHypBasic(grayscale_image,
                                                      rotation_hypotheses,
                                                      bounding_box,
                                                      clnf_model,
                                                      params);
        ...
        success = DetectLandmarksInImageMultiHypEarlyTerm(grayscale_image,
                                                          rotation_hypotheses,
                                                          bounding_box,
                                                          clnf_model,
                                                          params);
...
bool LandmarkDetector::DetectLandmarksInImage(const cv::Mat& rgb_image,
    ...
    if (params.curr_face_detector == FaceModelParameters::HOG_SVM_DETECTOR) {
        float confidence;
        LandmarkDetector::DetectSingleFaceHOG(bounding_box,
                                              grayscale_image,
                                              clnf_model.face_detector_HOG,
                                              confidence);
    }
    else if (params.curr_face_detector == FaceModelParameters::HAAR_DETECTOR) {
        LandmarkDetector::DetectSingleFace(
            bounding_box, rgb_image, clnf_model.face_detector_HAAR);
    }
    else if (params.curr_face_detector == FaceModelParameters::MTCNN_DETECTOR) {
        float confidence;
        LandmarkDetector::DetectSingleFaceMTCNN(bounding_box,
                                                rgb_image,
                                                clnf_model.face_detector_MTCNN,
                                                confidence);
    }
