...
void Grad(const cv::Mat& im, cv::Mat& grad) {
    ...
    cv::sepFilter2D(im, gradY, CV_32F, filterX.t(), filterY, cv::Point(-1,-1),
    0); cv::pow(gradX,2, gradX); cv::pow(gradY,2, gradY); grad = gradX + gradY;
...
void SVR_patch_expert::Response(const cv::Mat_<float>& area_of_interest,
    ...
    matchTemplate_m(normalised_area_of_interest,
                    empty_matrix_0,
                    empty_matrix_1,
                    empty_matrix_2,
                    weights,
...
void Multi_SVR_patch_expert::Response(const cv::Mat_<float>& area_of_interest,
    ...
    if (svr_patch_experts.size() == 1) {
        ...
        // responses from multiple patch experts these can be gradients, LBPs
        // etc.
        response.setTo(1.0);
