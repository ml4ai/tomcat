...
bool CLNF::Read_CLNF(string clnf_location) {
    ...
    boost::filesystem::path root =
        boost::filesystem::path(clnf_location).parent_path();
    ...
    while (!locations.eof()) {
        ...
        if (module.compare("PDM") == 0) {
            ...
            if (!triangulationFile.is_open()) {
                throw std::runtime_error(
                    "Could not read the Triangulations module from: " +
                    location + "....");
            }
    ...
    bool read_success = patch_experts.Read(intensity_expert_locations,
                                           ccnf_expert_locations,
                                           cen_expert_locations,
                                           early_term_loc);
...
void CLNF::Read(string main_location) {
    ...
    // main model
    boost::filesystem::path root =
        boost::filesystem::path(main_location).parent_path();
    ...
    while (!locations.eof()) {
        ...
        if (module.compare("LandmarkDetector") == 0) {
            ...
            std::string root_loc =
                boost::filesystem::path(main_location).parent_path().string();
...
bool CLNF::DetectLandmarks(const cv::Mat_<uchar>& image,
    ...
    // Fits from the current estimate of local and global parameters in the
    // model
    ...
    if (params.refine_hierarchical && hierarchical_models.size() > 0) {
        ...
        parallel_for_(
            cv::Range(0, hierarchical_models.size()),
            [&](const cv::Range& range) {
                for (int part_model = range.start; part_model < range.end;
                    ...
                    if (params_global[0] >
                        0.9 * hierarchical_models[part_model]
                                  .patch_experts.patch_scaling[0]) {
                        parts_used = true;

                        this->hierarchical_params[part_model]
                            .window_sizes_current =
                            this->hierarchical_params[part_model]
                                .window_sizes_init;
            ...
            for (size_t part_model = 0; part_model < hierarchical_models.size();
                ...
                for (size_t mapping_ind = 0; mapping_ind < mappings.size();
                    ...
                    detected_landmarks.at<float>(mappings[mapping_ind].first) =
                        hierarchical_models[part_model]
                            .detected_landmarks.at<float>(
                                mappings[mapping_ind].second);
                    detected_landmarks.at<float>(mappings[mapping_ind].first +
                                                 pdm.NumberOfPoints()) =
    ...
    if (params.validate_detections && fit_success) {
        ...
        cv::Vec3d orientation(
            params_global[1], params_global[2], params_global[3]);
...
bool CLNF::Fit(const cv::Mat_<float>& im,
    ...
    // Converting from image space to patch expert space (normalised for
    // rotation and scale)
    ...
    for (int scale = 0; scale < num_scales; scale++) {
        ...
        patch_experts.Response(patch_expert_responses,
                               sim_ref_to_img,
                               sim_img_to_ref,
                               im,
                               pdm,
        ...
        if (parameters.refine_parameters == true) {
            ...
            // Adapt the parameters based on scale (wan't to reduce
            // regularisation as scale increases, but increase sigma and
            // Tikhonov)
            tmp_parameters.reg_factor =
                parameters.reg_factor -
                15 * log(patch_experts.patch_scaling[scale_max] / 0.25) /
                    log(2);
            ...
            tmp_parameters.sigma =
                parameters.sigma +
                0.25 * log(patch_experts.patch_scaling[scale_max] / 0.25) /
                    log(2);
        ...
        if (scale == num_scales - 1 || window_sizes[scale + 1] == 0 ||
            ...
            this->model_likelihood = this->NU_RLMS(params_global,
                                                   params_local,
                                                   patch_expert_responses,
                                                   cv::Vec6f(params_global),
                                                   params_local.clone(),
        ...
        if (active_scale < num_scales - 1 &&
            0.9 * patch_experts.patch_scaling[active_scale + 1] <
                params_global[0])
...
void CLNF::NonVectorisedMeanShift_precalc_kde(
    ...
    for (int i = 0; i < n; i++) {
        ...
        // Pick the row from precalculated kde that approximates the current dx,
        // dy best
        int closest_col =
            (int)(dy / step_size +
        ...
        for (int ii = 0; ii < resp_size; ii++) {
            for (int jj = 0; jj < resp_size; jj++) {
                ...
                // the KDE evaluation of that point multiplied by the
                // probability at the current, xi, yi
                float v = (*p++) * (*kde_it++);
        ...
        for (int p = 0; p < n; p++) {
            if (!patch_experts.cen_expert_intensity.empty()) {
                ...
                WeightMatrix.at<float>(p, p) =
                    WeightMatrix.at<float>(p, p) +
                    patch_experts.cen_expert_intensity[scale][view_id][p]
                        .confidence;

                // for they y dimension
                WeightMatrix.at<float>(p + n, p + n) =
                    WeightMatrix.at<float>(p, p);
            }
...
float CLNF::NU_RLMS(cv::Vec6f& final_global,
    ...
    for (int iter = 0; iter < parameters.num_optimisation_iteration; iter++) {
        ...
        // behaviour is in 3D, using small angle approximation and oriented
        // shape
        if (rigid) {
        ...
        cv::Mat((current_shape_2D - base_shape_2D) *
                cv::Mat(sim_img_to_ref).t())
            .convertTo(offsets, CV_32F);

        dxs = offsets.col(0) + (resp_size - 1) / 2;
        dys = offsets.col(1) + (resp_size - 1) / 2;

        NonVectorisedMeanShift_precalc_kde(mean_shifts,
                                           patch_expert_responses,
                                           dxs,
                                           dys,
                                           resp_size,
        ...
        // Solve for the parameter update (from Baltrusaitis 2013 based on eq
        // (36) Saragih 2011)
    ...
    if (compute_lhood) {
        ...
        loglhood =
            loglhood / sum(patch_experts.visibilities[scale][view_id])[0];
    }
...
cv::Mat_<float> CLNF::GetShape(float fx, float fy, float cx, float cy) const {
    ...
    cv::Matx33f R =
        Utilities::Euler2RotationMatrix(cv::Vec3f((float)params_global[1],
                                                  (float)params_global[2],
