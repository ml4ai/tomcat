...
namespace LandmarkDetector {
    ...
    /**
        A Piece-wise Affine Warp
            The ideas for this piece-wise affine triangular warping are taken
       from the Active appearance models revisited by Iain Matthews and Simon
       Baker in IJCV 2004 This is used for both validation of landmark
       detection, and for avatar animation
    ...
    */
    ...
    class PAW {
      public:
        ...
        // Triangle index, indicating which triangle each of destination pixels
        // lies in
        cv::Mat_<int> triangle_id;

        // Indicating if the destination warped pixels is valid (lies within a
        // face)
        cv::Mat_<uchar> pixel_mask;

        // A number of precomputed coefficients that are helpful for quick
        // warping
        ...
        // The final optional argument allows for optimisation if the triangle
        // indices from previous frame are known (for tracking in video)
        PAW(const cv::Mat_<float>& destination_shape,
            const cv::Mat_<int>& triangulation,
            float in_min_x,
            float in_min_y,
        ...
        // The actual warping
        void Warp(const cv::Mat& image_to_warp,
      ...
      private:
        ...
        static bool sameSide(float x0,
                             ...
                             float x3,
                             float y3);
        static bool pointInTriangle(float x0,
                                    float y0,
                                    float x1,
                                    float y1,
                                    float x2,
                                    ...
                                    float y3);
        static int
        findTriangle(const cv::Point_<float>& point,
