...
namespace LandmarkDetector {
    ...
    class CLNF {
      ...
      public:
        ...
        // Global parameters describing the rigid shape [scale, euler_x,
        // euler_y, euler_z, tx, ty]
        cv::Vec6f params_global;

        // A collection of hierarchical CLNF models that can be used for
        // refinement
        ...
        // The actual output of the regressor (-1 is perfect detection 1 is
        // worst detection)
        ...
        //===========================================================================
        // Member variables that retain the state of the tracking (reflecting
        // the state of the lastly tracked (detected) image
        ...
        // when tracking in videos This is useful for knowing when to initialise
        // and reinitialise tracking
        int failures_in_a_row;

        // A template of a face that last succeeded with tracking (useful for
        // large motions in video)
        ...
        // Empty Destructor	as the memory of every object will be managed by
        // the corresponding libraries (no pointers)
        ~CLNF() {}
        ...
        // Gets the shape of the current detected landmarks in camera space
        // (given camera calibration) Can only be called after a call to
      ...
      private:
        ...
        // Mean shift computation that uses precalculated kernel density
        // estimators (the one actually used)
        ...
        // The actual model optimisation (update step), returns the model
        // likelihood
