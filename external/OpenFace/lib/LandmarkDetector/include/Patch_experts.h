...
namespace LandmarkDetector {
    ...
    */
    class Patch_experts {
      ...
      public:
        // The collection of SVR patch experts (for intensity/grayscale images),
        // the experts are laid out scale->view->landmark
        std::vector<std::vector<std::vector<Multi_SVR_patch_expert>>>
            svr_expert_intensity;

        // The collection of LNF (CCNF) patch experts (for intensity images),
        // the experts are laid out scale->view->landmark
        ...
        // The collection of CEN patch experts (for intensity images), the
        // experts are laid out scale->view->landmark
        std::vector<std::vector<std::vector<CEN_patch_expert>>>
            cen_expert_intensity;

        // Useful to pre-allocate data for im2col so that it is not allocated
        // for every iteration and every patch
        ...
        // Early termination calibration values, useful for CE-CLM model to
        // speed up the multi-hypothesis setup
        ...
        // response coordinates (and vice versa). The computation also requires
        // the current landmark locations to compute response around, the PDM
        // corresponding to the desired model, and the parameters describing its
        // instance Also need to provide the size of the area of interest and
