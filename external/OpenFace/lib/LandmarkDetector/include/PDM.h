...
namespace LandmarkDetector {
    //===========================================================================
    // A linear 3D Point Distribution Model (constructed using Non-Rigid
    // structure from motion or PCA) Only describes the model but does not
    // contain an instance of it (no local or global parameters are stored here)
    ...
    class PDM {
      public:
        ...
        // optional rotation), generates the global parameters that can generate
        // the face with the provided bounding box
        void CalcParams(cv::Vec6f& out_params_global,
                        const cv::Rect_<float>& bounding_box,
                        const cv::Mat_<float>& params_local,
                        const cv::Vec3f rotation = cv::Vec3f(0.0f));

        // Provided the landmark location compute global and local parameters
        // best fitting it (can provide optional rotation for potentially better
        // results)
        void CalcParams(cv::Vec6f& out_params_global,
