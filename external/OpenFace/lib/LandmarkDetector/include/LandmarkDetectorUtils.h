...
namespace LandmarkDetector {
    ...
    //===========================================================================
    // This is a modified version of openCV code that allows for precomputed
    // dfts of templates and for precomputed dfts of an image _img is the input
    // img, _img_dft it's dft (optional), _integral_img the images integral
    // image (optional), squared integral image (optional), templ is the
    // template we are convolving with, templ_dfts it's dfts at varying windows
    // sizes (optional), _result - the output, method the type of convolution
    ...
    // (as a 1D 2n x 1 vector of xs followed by doubles or as an n x 2
    // std::vector)
    ...
    std::vector<cv::Point2f>
    CalculateVisibleEyeLandmarks(const CLNF& clnf_model);

    std::vector<cv::Point2f>
    CalculateAllLandmarks(const cv::Mat_<float>& shape2D);
    ...
    bool
    DetectFaces(std::vector<cv::Rect_<float>>& o_regions,
    ...
    // present (pick the closest one), if it is not set the biggest face is
    // chosen
