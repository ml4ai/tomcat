...
cv::Point3f RaySphereIntersect(cv::Point3f rayOrigin,
                               cv::Point3f rayDir,
                               cv::Point3f sphereOrigin,
                               float sphereRadius) {
...
cv::Point3f GazeAnalysis::GetPupilPosition(cv::Mat_<float> eyeLdmks3d) {
    ...
    cv::Point3f p(mean(irisLdmks3d.col(0))[0],
...
void GazeAnalysis::EstimateGaze(const LandmarkDetector::CLNF& clnf_model,
    ...
    for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
        if (left_eye && clnf_model.hierarchical_model_names[i].compare(
                            "left_eye_28") == 0) {
            part = i;
        }
        if (!left_eye && clnf_model.hierarchical_model_names[i].compare(
    ...
    if (part == -1) {
        std::cout << "Couldn't find the eye model, something wrong"
                  << std::endl;
