...
namespace LandmarkDetector {
  ...
  void crossCorr_m(const cv::Mat_<float>& img,
    ...
    if (_templ_dfts.find(dftsize.width) == _templ_dfts.end()) {
      ...
      if (dst.cols > _templ.cols) {
        cv::Mat_<double> part(
            dst, cv::Range(0, _templ.rows), cv::Range(_templ.cols, dst.cols));
        part.setTo(0);
      }
  ...
  // a 1D 2n x 1 vector of xs followed by doubles or as an n x 2 vector)
  ...
  vector<cv::Point2f> CalculateVisibleLandmarks(const CLNF& clnf_model) {
    ...
    if (clnf_model.detection_success) {
      int idx =
          clnf_model.patch_experts.GetViewIdx(clnf_model.params_global, 0);
  ...
  vector<cv::Point2f> CalculateVisibleEyeLandmarks(const CLNF& clnf_model) {
    ...
    for (size_t i = 0; i < clnf_model.hierarchical_models.size(); ++i) {
      ...
      if (clnf_model.hierarchical_model_names[i].compare("left_eye_28") == 0 ||
        ...
        auto lmks =
            CalculateVisibleLandmarks(clnf_model.hierarchical_models[i]);
        for (auto lmk : lmks) {
  ...
  bool DetectFaces(vector<cv::Rect_<float>>& o_regions,
    ...
    for (size_t face = 0; face < face_detections.size(); ++face) {
      ...
      // CLNF detector expects the bounding box to encompass from eyebrow to
      // chin in y, and from cheeck outline to cheeck outline in x, so we need
      // to compensate
      ...
      // Shift face down as OpenCV Haar Cascade detects the forehead as well,
      // and we're not interested
      region.y =
          face_detections[face].y + face_detections[face].height * 0.2166f;
    ...
    if (detect_success) {
      ...
      if (face_detections.size() > 1) {
        ...
        for (size_t i = 0; i < face_detections.size(); ++i) {
          ...
          if (use_preferred) {
            dist = sqrt((preference.x) * (face_detections[i].width / 2 +
                                          face_detections[i].x) +
  ...
  bool DetectFacesHOG(vector<cv::Rect_<float>>& o_regions,
    ...
    for (size_t face = 0; face < face_detections.size(); ++face) {
      ...
      // and from cheeck outline to cheeck outline in x, so we need to
      // compensate
  ...
  bool DetectSingleFaceHOG(cv::Rect_<float>& o_region,
    ...
    if (detect_success) {
      ...
      // keep the most confident one or the one closest to preference point if
      // set
      float best_so_far;
      if (use_preferred) {
        best_so_far =
