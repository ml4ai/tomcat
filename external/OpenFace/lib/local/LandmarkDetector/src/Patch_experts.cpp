        ...
        if (view_id == 0) {
          ...
          if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
            ...
            if (mirror_id == ind) {
              ...
              // scale and rotate to mean shape to reference frame
              cv::Mat sim_r = (cv::Mat_<float>(2, 3) << a1,
                               -b1,
          ...
          if (!cen_expert_intensity[scale][view_id][ind].biases.empty()) {
            ...
            cv::Mat_<float> empty(0, 0, 0.0f);
            cen_expert_intensity[scale][mirror_views.at<int>(view_id)]
                                [mirror_inds.at<int>(ind)]
...
bool Patch_experts::Read(vector<string> intensity_svr_expert_locations,
  ...
  for (int scale = 0; scale < num_intensity_ccnf; ++scale) {
    ...
    cout << "Reading the intensity CCNF patch experts from: " << location
         << "....";
    bool success_read = Read_CCNF_patch_experts(location,
                                                centers[scale],
                                                visibilities[scale],
