...
vector<pair<string, bool>> FaceAnalyser::GetDynamicAUReg() const {
  ...
  for (size_t i = 0; i < au_reg_names_stat.size(); ++i) {
    au_dynamic_reg.push_back(pair<string, bool>(au_reg_names_stat[i], false));
  }
  for (size_t i = 0; i < au_reg_names_dyn.size(); ++i) {
...
void FaceAnalyser::AddNextFrame(const cv::Mat &frame,
  ...
  if (aligned_face_for_output.channels() == 3 && out_grayscale) {
    cvtColor(
        aligned_face_for_output, aligned_face_for_output, cv::COLOR_BGR2GRAY);
  }
...
void FaceAnalyser::ExtractAllPredictionsOfflineReg(
  ...
  for (auto au_iter = AU_predictions_reg_all_hist.begin();
    ...
    au_predictions.push_back(pair<string, vector<double>>(au_name, au_vals));
  ...
  if (!hog_desc_frame.empty()) {
    ...
    for (size_t i = 0; i < svr_lin_stat_preds.size(); ++i) {
      predictions.push_back(
          pair<string, double>(svr_lin_stat_aus[i], svr_lin_stat_preds[i]));
    }
...
vector<pair<string, double>> FaceAnalyser::PredictCurrentAUsClass(int view) {
  ...
  if (!hog_desc_frame.empty()) {
    ...
    for (size_t i = 0; i < svm_lin_stat_aus.size(); ++i) {
      predictions.push_back(
          pair<string, double>(svm_lin_stat_aus[i], svm_lin_stat_preds[i]));
    }
