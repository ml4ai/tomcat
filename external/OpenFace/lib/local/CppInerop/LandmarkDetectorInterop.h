  ...
  public
    ...
    public:
      ...
      CLNF(FaceModelParameters ^ params) {
        clnf =
            new ::LandmarkDetector::CLNF(params->getParams()->model_location);
      }
      ...
      bool DetectFaceLandmarksInImage(OpenCVWrappers::RawImage ^ rgb_image,
                                      ...
                                      FaceModelParameters ^ modelParams,
                                      OpenCVWrappers::RawImage ^ gray_image) {
        cv::Rect_<float> bbox(bounding_box->Left,
                              bounding_box->Top,
                              bounding_box->Width,
                              bounding_box->Height);
        return ::LandmarkDetector::DetectLandmarksInImage(
      ...
      void GetPoseWRTCamera(
          List<float> ^ pose, float fx, float fy, float cx, float cy) {
        auto pose_vec =
            ::LandmarkDetector::GetPoseWRTCamera(*clnf, fx, fy, cx, cy);
        pose->Clear();
        for (int i = 0; i < 6; ++i) {
          pose->Add(pose_vec[i]);
        }
      ...
      // Get the mask of which landmarks are currently visible (not
      // self-occluded)
      List<bool> ^
          GetVisibilities() {
          ...
          CalculateVisibleLandmarks() {
            vector<cv::Point2f> vecLandmarks =
          ...
          CalculateAllEyeLandmarks3D(float fx, float fy, float cx, float cy) {
            vector<cv::Point3f> vecLandmarks =
          ...
          Calculate3DLandmarks(float fx, float fy, float cx, float cy) {
            ...
            auto landmarks_3D =
                gcnew List<System::Tuple<float, float, float> ^>();

            for (int i = 0; i < shape3D.cols; ++i) {
              landmarks_3D->Add(gcnew System::Tuple<float, float, float>(
                  shape3D.at<float>(0, i),
                  shape3D.at<float>(1, i),
          ...
          CalculateBox(float fx, float fy, float cx, float cy) {
            ...
            vector<pair<cv::Point2f, cv::Point2f>> vecLines =
                ::Utilities::CalculateBox(pose, fx, fy, cx, cy);

            auto lines = gcnew List<System::Tuple<System::Windows::Point,
      ...
      List<float> ^
          ...
          // Getting the rigid shape parameters describing face scale rotation
          // and translation (scale,rotx,roty,rotz,tx,ty)
          List<float> ^
          GetRigidParams() {
