...
public
  ...
  public:
    ...
    SequenceReader(System::String ^ filename,
                   bool directory,
                   float fx,
                   float fy,
                   float cx,
      ...
      if (directory) {
        success =
            m_sequence_capture->OpenImageSequence(name_std, fx, fy, cx, cy);
      }
      else {
    ...
    SequenceReader(int webcam_id,
      ...
      bool success = m_sequence_capture->OpenWebcam(
          webcam_id, width, height, fx, fy, cx, cy);
  ...
  private:
    ...
    static Dictionary<System::String ^, List<System::Tuple<int, int> ^> ^> ^
        GetListingFromFile(std::string filename) {
          ...
          for (size_t idx = 0; idx < camera_node_list.size(); idx++) {
            std::string camera_name =
                (std::string)camera_node_list[idx]["name"];
      ...
      fs << "cameras"
         << "[";
                        for
                          each(System::String ^ name_m in camera_list->Keys) {

                            std::string name =
                                msclr::interop::marshal_as<std::string>(name_m);
          ...
          for (size_t i = 0; i < num_cameras; ++i) {
            ...
            if (camera_resolution_list->ContainsKey(device_name_m)) {
              ...
              for (size_t i = 0; i < common_resolutions.size(); ++i) {
                ...
                // Add only valid resolutions as API sometimes provides wrong
                // ones
                ...
                resolution =
                    gcnew System::Tuple<int, int>(set_width, set_height);
