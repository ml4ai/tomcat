...
namespace FaceAnalysis {
    ...
    class FaceAnalyser {
      ...
      public:
        ...
        // A standalone call for predicting AUs and computing face texture
        // features from a static image
      ...
      private:
        ...
        // Keeping track of AU predictions over time (useful for
        // post-processing)
        std::vector<double> timestamps;
        std::map<std::string, std::vector<double>> AU_predictions_reg_all_hist;
        std::map<std::string, std::vector<double>>
            AU_predictions_class_all_hist;
        ...
        // Using the bounding box of previous analysed frame to determine if a
        // reset is needed
        ...
        // A utility function for keeping track of approximate running medians
        // used for AU and emotion inference using a set of histograms (the
        // histograms are evenly spaced from min_val to max_val) Descriptor has
        // to be a row vector
        ...
        // The AUs predicted by the model are not always 0 calibrated to a
        // person. That is they don't always predict 0 for a neutral expression
        // Keeping track of the predictions we can correct for this, by assuming
        // that at least "ratio" of frames are neutral and subtract that value
        // of prediction, only perform the correction after min_frames
        ...
        // Some dynamic scaling (the logic is that before the extreme versions
        // of expression or emotion are shown, it is hard to tell the
        // boundaries, this allows us to scale the model to the most extreme
        // seen) They have to be view specific
        ...
        // Useful placeholder for renormalizing the initial frames of shorter
        // videos
