...
namespace FaceAnalysis {
    ...
    void AlignFace(cv::Mat& aligned_face,
        ...
        cv::Mat_<float> similarity_normalised_shape =
            pdm.mean_shape * sim_scale;
    ...
    void AlignFaceMask(cv::Mat& aligned_face,
        ...
        cv::Matx22f warp_matrix_2d(warp_matrix(0, 0),
                                   warp_matrix(0, 1),
                                   warp_matrix(1, 0),
                                   warp_matrix(1, 1));
        ...
        destination_landmarks =
            cv::Mat(destination_landmarks.t()).reshape(1, 1).t();

        LandmarkDetector::PAW paw(destination_landmarks,
                                  triangulation,
                                  0,
                                  0,
                                  aligned_face.cols - 1,
        ...
        // Mask each of the channels (a bit of a roundabout way, but OpenCV 3.1
        // in debug mode doesn't seem to be able to handle a more direct way
        ...
        for (size_t i = 0; i < aligned_face_channels.size(); ++i) {
            cv::multiply(aligned_face_channels[i],
                         paw.pixel_mask,
                         aligned_face_channels[i],
                         1.0,
                         CV_8U);
        }

        if (aligned_face.channels() == 3) {
            cv::Mat planes[] = {aligned_face_channels[0],
                                aligned_face_channels[1],
    ...
    // Extract summary statistics (mean, stdev, min, max) from each dimension of
    // a descriptor, each row is a descriptor
