...
vector<string> FaceAnalyser::GetAUClassNames() const {
    vector<string> au_class_names_all;
    vector<string> au_class_names_stat =
        AU_SVM_static_appearance_lin.GetAUNames();
    vector<string> au_class_names_dyn =
...
vector<string> FaceAnalyser::GetAURegNames() const {
    vector<string> au_reg_names_all;
    vector<string> au_reg_names_stat =
        AU_SVR_static_appearance_lin_regressors.GetAUNames();
    vector<string> au_reg_names_dyn =
...
vector<bool> FaceAnalyser::GetDynamicAUClass() const {
    vector<bool> au_dynamic_class;
    vector<string> au_class_names_stat =
        AU_SVM_static_appearance_lin.GetAUNames();
    vector<string> au_class_names_dyn =
        AU_SVM_dynamic_appearance_lin.GetAUNames();
...
vector<pair<string, bool>> FaceAnalyser::GetDynamicAUReg() const {
    vector<pair<string, bool>> au_dynamic_reg;
    vector<string> au_reg_names_stat =
        AU_SVR_static_appearance_lin_regressors.GetAUNames();
    vector<string> au_reg_names_dyn =
        AU_SVR_dynamic_appearance_lin_regressors.GetAUNames();
    ...
    for (size_t i = 0; i < au_reg_names_stat.size(); ++i) {
        au_dynamic_reg.push_back(
            pair<string, bool>(au_reg_names_stat[i], false));
    }
    for (size_t i = 0; i < au_reg_names_dyn.size(); ++i) {
        au_dynamic_reg.push_back(pair<string, bool>(au_reg_names_dyn[i], true));
    }
...
void FaceAnalyser::PredictStaticAUsAndComputeFeatures(
    ...
    AlignFaceMask(aligned_face_for_au,
                  frame,
                  detected_landmarks,
                  params_global,
                  pdm,
    ...
    if (align_scale_out == align_scale_au &&
        ...
        if (align_mask) {
            AlignFaceMask(aligned_face_for_output,
                          frame,
                          detected_landmarks,
                          params_global,
                          pdm,
            ...
            AlignFace(aligned_face_for_output,
                      frame,
                      detected_landmarks,
                      params_global,
                      pdm,
    ...
    Extract_FHOG_descriptor(hog_descriptor,
                            aligned_face_for_au,
                            this->num_hog_rows,
                            this->num_hog_cols);
    ...
    // TODO rem cv::Mat_<uchar> aligned_face_cols(1, aligned_face_for_au.cols *
...
void FaceAnalyser::AddNextFrame(const cv::Mat& frame,
    ...
    if (success) {
        ...
        // If the aligned face for AU matches the output requested one, just
        // reuse it, else compute it
        if (align_scale_out == align_scale_au &&
            align_width_out == align_width_au &&
            align_height_out == align_height_au && align_mask) {
            aligned_face_for_output = aligned_face_for_au.clone();
    ...
    if (aligned_face_for_output.channels() == 3 && out_grayscale) {
        cvtColor(aligned_face_for_output,
                 aligned_face_for_output,
                 cv::COLOR_BGR2GRAY);
    }
    ...
    for (size_t au = 0; au < AU_predictions_class.size(); ++au) {
        ...
        if (success) {
            AU_predictions_class_all_hist[AU_predictions_class[au].first]
                .push_back(AU_predictions_class[au].second);
        }
        else {
            AU_predictions_class_all_hist[AU_predictions_class[au].first]
                .push_back(0);
    ...
    vector<pair<string, double>> AU_predictions_reg_corrected;
    if (online) {
    ...
    if (!postprocessed) {
        ...
        while (all_ind < all_frames_size && success_ind < max_init_frames) {
            ...
            if (valid_preds[all_ind]) {
                ...
                this->geom_descriptor_frame =
                    geom_descriptor_frames_init[success_ind];
                ...
                for (size_t au = 0; au < AU_predictions_reg.size(); ++au) {
                    // Find the appropriate AU (if not found add it)
                    AU_predictions_reg_all_hist[AU_predictions_reg[au]
                                                    .first][all_ind] =
                        AU_predictions_reg[au].second;
                }

                auto AU_predictions_class =
                    PredictCurrentAUsClass(views[success_ind]);

                for (size_t au = 0; au < AU_predictions_class.size(); ++au) {
                    // Find the appropriate AU (if not found add it)
                    AU_predictions_class_all_hist[AU_predictions_class[au]
                                                      .first][all_ind] =
...
void FaceAnalyser::ExtractAllPredictionsOfflineReg(
    ...
    vector<string> dyn_au_names =
        AU_SVR_dynamic_appearance_lin_regressors.GetAUNames();

    // Allow these AUs to be person calirated based on expected number of
    // neutral frames (learned from the data)
    for (auto au_iter = AU_predictions_reg_all_hist.begin();
         au_iter != AU_predictions_reg_all_hist.end();
         ++au_iter) {
        ...
        au_predictions.push_back(
            pair<string, vector<double>>(au_name, au_vals));
        ...
        if (au_good.empty() || !dynamic) {
            ...
            sort(au_good.begin(), au_good.end());
            // If it is a dynamic AU regressor we can also do some prediction
            // shifting to make it more accurate The shifting proportion is
            // learned and is callen cutoff
            ...
            if (au_id != -1 &&
                AU_SVR_dynamic_appearance_lin_regressors.GetCutoffs()[au_id] !=
                    -1) {
                double cutoff = AU_SVR_dynamic_appearance_lin_regressors
                                    .GetCutoffs()[au_id];
                offsets.push_back(
                    au_good.at((int)((double)au_good.size() * cutoff)));
            }
            else {
                offsets.push_back(0);
            }
    ...
    for (size_t au = 0; au < au_predictions.size(); ++au) {
        ...
        for (size_t frame = 0; frame < au_predictions[au].second.size();
             ++frame) {
    ...
    for (auto au_iter = au_predictions.begin(); au_iter != au_predictions.end();
        ...
        for (size_t i = (window_size - 1) / 2;
             i < au_iter->second.size() - (window_size - 1) / 2;
             ++i) {
            double sum = 0;
            for (int w = -(window_size - 1) / 2; w <= (window_size - 1) / 2;
                 ++w) {
                sum += au_vals_tmp[i + w];
            }
            sum = sum / window_size;
...
void FaceAnalyser::ExtractAllPredictionsOfflineClass(
    ...
    for (auto au_iter = AU_predictions_class_all_hist.begin();
         au_iter != AU_predictions_class_all_hist.end();
         ++au_iter) {
        string au_name = au_iter->first;
        vector<double> au_vals = au_iter->second;
        ...
        if ((int)au_vals.size() > (window_size - 1) / 2) {
            for (size_t i = (window_size - 1) / 2;
                 i < au_vals.size() - (window_size - 1) / 2;
                 ++i) {
                double sum = 0;
                int div_by = 0;
                for (int w = -(window_size - 1) / 2;
                     w <= (window_size - 1) / 2 && (i + w < au_vals_tmp.size());
                     ++w) {
                    sum += au_vals_tmp[i + w];
                    div_by++;
                }
...
void FaceAnalyser::Reset() {
    ...
    for (size_t i = 0; i < hog_desc_hist.size(); ++i) {
        ...
        this->face_image_hist[i] = cv::Mat_<int>(
            face_image_hist[i].rows, face_image_hist[i].cols, (int)0);
        ...
        this->au_prediction_correction_histogram[i] =
            cv::Mat_<int>(au_prediction_correction_histogram[i].rows,
                          au_prediction_correction_histogram[i].cols,
                          (int)0);
    }
    ...
    AU_prediction_track = cv::Mat_<double>(
        AU_prediction_track.rows, AU_prediction_track.cols, 0.0);

    geom_desc_track =
        cv::Mat_<double>(geom_desc_track.rows, geom_desc_track.cols, 0.0);

    dyn_scaling = vector<vector<double>>(
        dyn_scaling.size(), vector<double>(dyn_scaling[0].size(), 5.0));
        ...
        for (int i = 0; i < histogram.rows; ++i) {
            ...
            for (int j = 0; j < histogram.cols; ++j) {
                ...
                if (cummulative_sum >= cutoff_point) {
                    median.at<double>(i) =
                        min_val + ((double)j) * (length / ((double)num_bins)) +
    ...
    if (histogram.empty()) {
        ...
        for (int i = 0; i < histogram.rows; ++i) {
            ...
            for (int j = 0; j < histogram.cols; ++j) {
                cummulative_sum += histogram.at<int>(i, j);
                if (cummulative_sum > cutoff_point) {
                    median.at<double>(i) = min_val + j * (max_val / num_bins) +
                                           (0.5 * (length) / num_bins);
                    break;
                }
            }
...
vector<pair<string, double>> FaceAnalyser::PredictCurrentAUs(int view) {
    ...
    if (!hog_desc_frame.empty()) {
        ...
        AU_SVR_static_appearance_lin_regressors.Predict(svr_lin_stat_preds,
                                                        svr_lin_stat_aus,
                                                        hog_desc_frame,
                                                        geom_descriptor_frame);

        for (size_t i = 0; i < svr_lin_stat_preds.size(); ++i) {
            predictions.push_back(pair<string, double>(svr_lin_stat_aus[i],
                                                       svr_lin_stat_preds[i]));
        }
        ...
        for (size_t i = 0; i < svr_lin_dyn_preds.size(); ++i) {
            predictions.push_back(
                pair<string, double>(svr_lin_dyn_aus[i], svr_lin_dyn_preds[i]));
        }
    }
...
FaceAnalyser::CorrectOnlineAUs(vector<pair<string, double>> predictions_orig,
                               int view,
                               bool dyn_shift,
                               bool dyn_scale,
                               bool update_track,
    ...
    if (dyn_scale) {
        ...
        // Also makes sense as till the maximum expression is seen, it is hard
        // to tell how expressive a persons face is
        if (dyn_scaling[view].empty()) {
            dyn_scaling[view] = vector<double>(predictions.size(), 5.0);
        }

        for (size_t i = 0; i < predictions.size(); ++i) {
            // First establish presence (assume it is maximum as we have not
            // seen max)
            if (predictions[i].second > 1) {
                ...
                predictions[i].second =
                    predictions[i].second * dyn_scaling[view][i];
            }
...
vector<pair<string, double>> FaceAnalyser::PredictCurrentAUsClass(int view) {
    ...
    if (!hog_desc_frame.empty()) {
        ...
        AU_SVM_static_appearance_lin.Predict(svm_lin_stat_preds,
                                             svm_lin_stat_aus,
                                             hog_desc_frame,
                                             geom_descriptor_frame);

        for (size_t i = 0; i < svm_lin_stat_aus.size(); ++i) {
            predictions.push_back(pair<string, double>(svm_lin_stat_aus[i],
                                                       svm_lin_stat_preds[i]));
        }

        vector<string> svm_lin_dyn_aus;
        vector<double> svm_lin_dyn_preds;

        AU_SVM_dynamic_appearance_lin.Predict(svm_lin_dyn_preds,
                                              svm_lin_dyn_aus,
                                              hog_desc_frame,
                                              geom_descriptor_frame,
                                              this->hog_desc_median,
        ...
        for (size_t i = 0; i < svm_lin_dyn_aus.size(); ++i) {
            predictions.push_back(
                pair<string, double>(svm_lin_dyn_aus[i], svm_lin_dyn_preds[i]));
        }
    }
...
void FaceAnalyser::Read(string model_loc) {
    ...
    // main model
    path root = path(model_loc).parent_path();
...
void FaceAnalyser::ReadAU(string au_model_location) {

    // Open the list of the regressors in the file
    ifstream locations(au_model_location.c_str(), ios::in);

    if (!locations.is_open()) {
        cout << "Couldn't open the AU prediction files at: "
             << au_model_location.c_str() << " aborting" << endl;
        cout.flush();
        return;
    }
    ...
    // The other module locations should be defined as relative paths from the
    // main model
    path root = path(au_model_location).parent_path();
    ...
    while (!locations.eof()) {
        ...
        if (index >= 0) {
            ...
            // remove carriage return at the end for compatibility with unix
            // systems
            name.erase(name.find_last_not_of(" \n\r\t") + 1);
        }
        vector<string> au_names;
        split(au_names, name, is_any_of(","));
...
void FaceAnalyser::UpdatePredictionTrack(
    ...
    for (int i = 0; i < prediction_corr_histogram.rows; ++i) {
        // Find the bins corresponding to the current descriptor
        int index = (int)((predictions[i].second - min_val) *
                          ((double)num_bins) / (length));
        if (index < 0) {
            index = 0;
        }
...
void FaceAnalyser::ReadRegressor(string fname, const vector<string>& au_names) {
    ...
    if (regressor_stream.is_open()) {
        ...
        if (regressor_type == SVR_appearance_static_linear) {
            AU_SVR_static_appearance_lin_regressors.Read(regressor_stream,
                                                         au_names);
        }
        else if (regressor_type == SVR_appearance_dynamic_linear) {
            AU_SVR_dynamic_appearance_lin_regressors.Read(regressor_stream,
                                                          au_names);
        }
...
void FaceAnalyser::PostprocessOutputFile(string output_file) {
    ...
    vector<bool> successes;
    vector<double> timestamps;
    vector<pair<string, vector<double>>> predictions_reg;
    vector<pair<string, vector<double>>> predictions_class;

    // Construct the new values to overwrite the output file with
    ExtractAllPredictionsOfflineReg(
        predictions_reg, certainties, successes, timestamps, dynamic);
    ExtractAllPredictionsOfflineClass(
        predictions_class, certainties, successes, timestamps, dynamic);
    ...
    split(tokens, output_file_contents[0], is_any_of(","));
    ...
    ofstream outfile(output_file, ios_base::out);
    // Write the header
    ...
    for (int i = 1; i < (int)output_file_contents.size(); ++i) {
        vector<string> tokens;
        split(tokens, output_file_contents[i], is_any_of(","));
        ...
        for (int t = 1; t < (int)tokens.size(); ++t) {
            if (t >= begin_ind && t < end_ind) {
                if (t - begin_ind < num_reg) {
                    outfile << ", "
                            << predictions_reg[inds_reg[t - begin_ind]]
                                   .second[i - 1];
                }
                else {
                    outfile << ", "
                            << predictions_class[inds_class[t - begin_ind -
                                                            num_reg]]
                                   .second[i - 1];
                }
            }
