...
void CreateDirectory(std::string output_path) {
    // Removing trailing separators, as that causes issues with directory
    // creation in unix
    ...
    if (!boost::filesystem::exists(p)) {
        ...
        if (!success) {
            std::cout << "ERROR: failed to create output directory:"
                      << p.string()
                      << ", do you have permission to create directory"
                      << std::endl;
            exit(1);
        }
    }
...
void RecorderOpenFace::VideoWritingTask(bool is_sequence) {
    ...
    while (true) {
        ...
        if (is_sequence) {
            ...
            bool out_success =
                cv::imwrite(tracked_data.first, tracked_data.second);
            if (!out_success) {
...
void RecorderOpenFace::PrepareRecording(const std::string& in_filename) {
    ...
    if (!metadata_file.is_open()) {
        cout << "ERROR: could not open the output file:" << of_det_name
             << ", either the path of the output directory is wrong or you do "
                "not "
                "have the permissions to write to it"
             << endl;
        exit(1);
    }
    ...
    if (!params.isFromWebcam()) {
        ...
        if (!boost::filesystem::path(input_filename_full).is_absolute()) {
            input_filename_full =
                boost::filesystem::canonical(input_filename_relative).string();
        }
    ...
    if (params.outputHOG()) {
        // Output the data based on record_root, but do not include record_root
        // in the meta file, as it is also in that directory
    ...
    if (params.outputTracked()) {
        if (params.isSequence()) {
            // Output the data based on record_root, but do not include
            // record_root in the meta file, as it is also in that directory
            this->media_filename = out_name + ".avi";
            metadata_file << "Output video:" << this->media_filename << endl;
            this->media_filename =
                (path(record_root) / this->media_filename).string();
        }
        else {
            this->media_filename =
                out_name + "." + params.imageFormatVisualization();
    ...
    if (params.outputAlignedFaces()) {
        ...
        metadata_file << "Output aligned directory:"
                      << this->aligned_output_directory << endl;
        this->aligned_output_directory =
            (path(record_root) / this->aligned_output_directory).string();
...
RecorderOpenFace::RecorderOpenFace(const std::string in_filename,
    ...
    if (boost::filesystem::is_directory(in_filename)) {
        out_name =
            boost::filesystem::canonical(boost::filesystem::path(in_filename))
                .filename()
                .string();
    }
    else {
        out_name = boost::filesystem::path(in_filename)
                       .filename()
                       .replace_extension("")
                       .string();
    }
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        if (!output_found && arguments[i].compare("-of") == 0) {
            record_root = (boost::filesystem::path(record_root) /
                           boost::filesystem::path(arguments[i + 1]))
                              .remove_filename()
                              .string();
            out_name = path(boost::filesystem::path(arguments[i + 1]))
                           .replace_extension("")
                           .filename()
                           .string();
            valid[i] = false;
...
void RecorderOpenFace::WriteObservation() {
    ...
    if (!csv_recorder.isOpen()) {
        ...
        csv_recorder.Open(csv_filename,
                          params.isSequence(),
    ...
    this->csv_recorder.WriteLine(face_id,
                                 frame_number,
                                 timestamp,
    ...
    if (params.outputAlignedFaces()) {
        ...
        string out_file = aligned_output_directory + preferredSlash +
                          string(name) + params.imageFormatAligned();
...
void RecorderOpenFace::WriteObservationTracked() {
    ...
    if (params.outputTracked()) {
        ...
        if (!tracked_writing_thread_started) {
            ...
            int capacity =
                (1024 * 1024 * TRACKED_QUEUE_CAPACITY) /
            ...
            if (params.isSequence()) {
                ...
                try {
                    video_writer.open(media_filename,
                                      cv::VideoWriter::fourcc(output_codec[0],
                                                              output_codec[1],
                                                              output_codec[2],
                                                              output_codec[3]),
                                      params.outputFps(),
                    ...
                    if (!video_writer.isOpened()) {
                        WARN_STREAM("Could not open VideoWriter, OUTPUT FILE "
                                    "WILL NOT BE WRITTEN.");
                    }
                }
                catch (cv::Exception e) {
                    WARN_STREAM(
                        "Could not open VideoWriter, OUTPUT FILE WILL NOT BE "
            ...
            video_writing_thread = std::thread(
                &RecorderOpenFace::VideoWritingTask, this, params.isSequence());
        }
        ...
        if (params.isSequence()) {
            vis_to_out_queue.push(
                std::pair<std::string, cv::Mat>("", vis_to_out));
        }
        else {
            vis_to_out_queue.push(
...
void RecorderOpenFace::SetObservationHOG(bool good_frame,
                                         ...
                                         int num_rows,
                                         int num_channels) {
    this->hog_recorder.SetObservationHOG(
        good_frame, hog_descriptor, num_cols, num_rows, num_channels);
}
