...
bool ImageCapture::Open(std::vector<std::string>& arguments) {
    ...
    for (size_t i = 0; i < arguments.size(); ++i) {
        if (arguments[i].compare("-f") == 0) {
            ...
            if (directory_found) {
                WARN_STREAM(
                    "Input directory already found, using the first one:" +
                    input_directory);
            }
            else {
...
bool ImageCapture::OpenDirectory(std::string directory,
    ...
    for (std::vector<boost::filesystem::path>::const_iterator file_iterator(
        ...
        if (file_iterator->extension().string().compare(".jpg") == 0 ||
            ...
            // If bounding box directory is specified, read the bounding boxes
            // from it
            if (!bbox_directory.empty()) {
                boost::filesystem::path current_file = *file_iterator;
                boost::filesystem::path bbox_file =
                    bbox_directory /
                    current_file.filename().replace_extension("txt");

                // If there is a bounding box file push it to the list of
                // bounding boxes
                if (boost::filesystem::exists(bbox_file)) {
                    std::ifstream in_bbox(bbox_file.string().c_str(),
                                          std::ios_base::in);

                    std::vector<cv::Rect_<float>> bboxes_image;

                    // Keep reading bounding boxes from a file, stop if empty
                    // line or
                    while (!in_bbox.eof()) {
                        ...
                        bboxes_image.push_back(cv::Rect_<float>(
                    ...
                    ERROR_STREAM("Could not find the corresponding bounding "
                                 "box for file:" +
    ...
    if (image_files.empty()) {
        std::cout << "No images found in the directory: " << directory
                  << std::endl;
        return false;
    }
