...
const map<string, string> AUS_DESCRIPTION = {
...
Visualizer::Visualizer(vector<string> arguments) {
...
Visualizer::Visualizer(bool vis_track,
                       bool vis_hog,
                       bool vis_align,
                       bool vis_aus) {
        ...
        for (int i = 0; i < n; ++i) {
            if (visibilities.empty() || visibilities.at<int>(i)) {
                cv::Point featurePoint(
                    ...
                    cvRound(landmarks_2D.at<float>(i + n) *
                            (float)draw_multiplier));

                // A rough heuristic for drawn point size
                int thickness =
                    (int)ceil(3.0 * ((double)captured_image.cols) / 640.0);
                int thickness_2 =
                    (int)ceil(1.0 * ((double)captured_image.cols) / 640.0);

                cv::circle(captured_image,
                           featurePoint,
                           1 * draw_multiplier,
                           cv::Scalar(0, 0, 255),
                           thickness,
                ...
                cv::Point featurePoint(cvRound(landmarks_2D.at<float>(i) *
                                               (double)draw_multiplier),
                                       cvRound(landmarks_2D.at<float>(i + n) *
                                               (double)draw_multiplier));

                // A rough heuristic for drawn point size
                int thickness =
                    (int)ceil(2.5 * ((double)captured_image.cols) / 640.0);
                int thickness_2 =
                    (int)ceil(1.0 * ((double)captured_image.cols) / 640.0);
...
void Visualizer::SetObservationPose(const cv::Vec6f& pose, double confidence) {
    ...
    if (confidence > visualisation_boundary) {
        ...
        // Scale from 0 to 1, to allow to indicated by colour how confident we
        // are in the tracking
        vis_certainty = (vis_certainty - visualisation_boundary) /
                        (1 - visualisation_boundary);

        // A rough heuristic for box around the face width
        int thickness = (int)ceil(2.0 * ((double)captured_image.cols) / 640.0);

        // Draw it in reddish if uncertain, blueish if certain
        DrawBox(captured_image,
                pose,
...
void Visualizer::SetObservationActionUnits(
    ...
    if (au_intensities.size() > 0 || au_occurences.size() > 0) {
        ...
        set<string> au_names;
        map<string, bool> occurences_map;
        map<string, double> intensities_map;

        for (size_t idx = 0; idx < au_intensities.size(); idx++) {
            au_names.insert(au_intensities[idx].first);
            intensities_map[au_intensities[idx].first] =
                au_intensities[idx].second;
        }

        for (size_t idx = 0; idx < au_occurences.size(); idx++) {
            au_names.insert(au_occurences[idx].first);
            occurences_map[au_occurences[idx].first] =
                au_occurences[idx].second > 0;
        }
        ...
        if (action_units_image.empty()) {
            action_units_image =
                cv::Mat(nb_aus * (AU_TRACKBAR_HEIGHT + 10) + MARGIN_Y * 2,
        ...
        map<string, pair<bool, double>> aus;

        // first, prepare a mapping "AU name" -> { present, intensity }
        for (auto au_name : au_names) {
            // Insert the intensity and AU presense (as these do not always
            // overlap check if they exist first)
            ...
            aus[au_name] = make_pair(occurence, intensity);
        }

        // then, build the graph
        unsigned int idx = 0;
        for (auto& au : aus) {
            string name = au.first;
            bool present = au.second.first;
            double intensity = au.second.second;

            int offset = MARGIN_Y + idx * (AU_TRACKBAR_HEIGHT + 10);
            ostringstream au_i;
            au_i << setprecision(2) << setw(4) << fixed << intensity;
            cv::putText(action_units_image,
                        name,
                        cv::Point(10, offset + 10),
                        cv::FONT_HERSHEY_SIMPLEX,
                        0.5,
                        CV_RGB(present ? 0 : 200, 0, 0),
                        1,
                        cv::LINE_AA);
            cv::putText(action_units_image,
                        AUS_DESCRIPTION.at(name),
                        cv::Point(55, offset + 10),
                        cv::FONT_HERSHEY_SIMPLEX,
                        0.3,
                        CV_RGB(0, 0, 0),
                        1,
            ...
            if (present) {
                cv::putText(action_units_image,
                            ...
                            1,
                            cv::LINE_AA);
                cv::rectangle(action_units_image,
                              cv::Point(MARGIN_X, offset),
                              cv::Point((int)(MARGIN_X + AU_TRACKBAR_LENGTH *
                                                             intensity / 5.0),
...
void Visualizer::SetObservationGaze(const cv::Point3f& gaze_direction0,
                                    const cv::Point3f& gaze_direction1,
                                    const vector<cv::Point2f>& eye_landmarks2d,
                                    const vector<cv::Point3f>& eye_landmarks3d,
                                    double confidence) {
    if (confidence > visualisation_boundary) {
        if (eye_landmarks2d.size() > 0) {
            ...
            for (size_t i = 0; i < eye_landmarks2d.size(); ++i) {
                ...
                cv::Point nextFeaturePoint(
                    cvRound(eye_landmarks2d[next_point].x *
                            (double)draw_multiplier),
                    cvRound(eye_landmarks2d[next_point].y *
                            (double)draw_multiplier));
                if ((i < 28 && (i < 8 || i > 19)) ||
                    (i >= 28 && (i < 8 + 28 || i > 19 + 28)))
                    cv::line(captured_image,
                             featurePoint,
                             nextFeaturePoint,
                             cv::Scalar(255, 0, 0),
                             thickness_2,
            ...
            // Grabbing the pupil location, to draw eye gaze need to know where
            // the pupil is
            ...
            for (size_t i = 0; i < 8; ++i) {
                ...
                pupil_right = pupil_right +
                              eye_landmarks3d[i + eye_landmarks3d.size() / 2];
            }
            ...
            cv::Mat_<float> mesh_0 = (cv::Mat_<float>(2, 3) << points_left[0].x,
                                      points_left[0].y,
                                      points_left[0].z,
                                      points_left[1].x,
                                      points_left[1].y,
            ...
            cv::line(captured_image,
                     cv::Point(cvRound(proj_points.at<float>(0, 0) *
                                       (float)draw_multiplier),
                               cvRound(proj_points.at<float>(0, 1) *
                                       (float)draw_multiplier)),
                     cv::Point(cvRound(proj_points.at<float>(1, 0) *
                                       (float)draw_multiplier),
                               cvRound(proj_points.at<float>(1, 1) *
                                       (float)draw_multiplier)),
                     cv::Scalar(110, 220, 0),
                     2,
                     cv::LINE_AA,
            ...
            cv::Mat_<float> mesh_1 =
                (cv::Mat_<float>(2, 3) << points_right[0].x,
                 points_right[0].y,
                 points_right[0].z,
                 points_right[1].x,
                 points_right[1].y,
