...
namespace Utilities {
    ...
    //===========================================================================
    /**
    A class for recording data processed by OpenFace (facial landmarks, head
    pose, facial action units, aligned face, HOG features, and tracked video
    */
    class RecorderOpenFace {

      public:
        // The constructor for the recorder, need to specify if we are recording
        // a sequence or not, in_filename should be just the name and not
        // contain extensions
        ...
        // Separate method for writing tracked video observation, this is done
        // because video observation is written once a frame/image, other
        // observations can happen multiple times a frame/image
        void WriteObservationTracked();

        std::string GetCSVFile() { return csv_filename; }

      private:
        // Blocking copy, assignment and move operators, as it does not make
        // sense to save to the same location
        ...
        std::string default_record_directory =
            "processed"; // By default we are writing in the processed directory
                         // in the working directory, if no output parameters
                         // provided
        std::string
