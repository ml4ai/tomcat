...
public class Room extends CompositeDrawingObject {
  ...
  public void createRoom(int x,
                         ...
                         boolean withRoof) {
    Plane west_wall =
        new Plane(x, y, z, THICKNESS_OF_THE_WALLS, height, depth + 2, type);
    Plane north_wall = new Plane(
        x - 1, y, z + depth + 1, width, height, THICKNESS_OF_THE_WALLS, type);
    Plane east_wall = new Plane(
        x - width - 1, y, z, THICKNESS_OF_THE_WALLS, height, depth + 2, type);
    Plane south_wall =
        new Plane(x - 1, y, z, width, height, THICKNESS_OF_THE_WALLS, type);
    ...
    if (withRoof) {
      Plane roof = new Plane(
          x, y + height, z, width + 2, THICKNESS_OF_THE_WALLS, depth, type);
      this.mergeWith(roof);
    }
  }
}
