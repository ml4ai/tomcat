...
public class AnimationDrawingHelper extends BlockDrawingHelper {
  ...
  public void endDrawing(World w) { super.endDrawing(w); }
  ...
  public Vec3d getMin() { return this.minPos; }

  public Vec3d getMax() { return this.maxPos; }

  public Vec3d getOrigin() { return this.origin; }

  public void setOrigin(Vec3d org) { this.origin = org; }

  @Override
  public void setBlockState(World w, BlockPos pos, XMLBlockState state) {
    BlockPos offsetPos =
        pos.add(this.origin.xCoord, this.origin.yCoord, this.origin.zCoord);
    this.drawing.add(offsetPos);
    this.previousFrame.remove(offsetPos);
    if (this.minPos == null)
      this.minPos = new Vec3d(
          offsetPos.getX() - 0.5, offsetPos.getY(), offsetPos.getZ() - 0.5);
    else {
      ...
      if (x != this.minPos.xCoord || y != this.minPos.yCoord ||
          z != this.minPos.zCoord)
        this.minPos = new Vec3d(x, y, z);
    }
    if (this.maxPos == null)
      this.maxPos = new Vec3d(
          offsetPos.getX() + 0.5, offsetPos.getY() + 1, offsetPos.getZ() + 0.5);
    else {
      ...
      if (x != this.maxPos.xCoord || y != this.maxPos.yCoord ||
          z != this.maxPos.zCoord)
  ...
  public void clearEntities(World w,
                            ...
                            double y2,
                            double z2) {
    super.clearEntities(w,
                        x1 + this.origin.xCoord,
                        y1 + this.origin.yCoord,
                        z1 + this.origin.zCoord,
                        x2 + this.origin.xCoord,
                        y2 + this.origin.yCoord,
                        z2 + this.origin.zCoord);
  }
  ...
  protected EntityItem createItem(ItemStack stack,
    ...
    return super.createItem(stack,
                            x + this.origin.xCoord,
                            y + this.origin.yCoord,
                            z + this.origin.zCoord,
                            w,
                            centreItem);
  }
