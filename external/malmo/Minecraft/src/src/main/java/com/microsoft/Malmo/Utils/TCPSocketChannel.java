...
public class TCPSocketChannel {
  ...
  private void Log(Level level, String message) {
    TCPUtils.Log(level,
                 "<-" + this.logname + "(" + this.address + ":" + this.port +
                     ") " + message);
  }
   ...
   */
  public boolean sendTCPBytes(byte[] buffer) { return sendTCPBytes(buffer, 0); }
  ...
  public boolean sendTCPBytes(byte[] bytes, int retries) {
    try {
      ...
      SysLog(Level.SEVERE,
             "Failed to send TCP bytes" + (retries > 0 ? " -- retrying " : "") +
                 ": " + e);
  ...
  public boolean sendTCPBytes(ByteBuffer[] srcbuffers, int length) {
    ...
    try {
      ...
      if (TCPUtils.isLogging()) {
        ...
        double rate = 1000.0 * 1000.0 * 1000.0 * (double)(bytesWritten) /
                      (1024.0 * (double)(t2 - t1));
  ...
  private void safeWrite(ByteBuffer buffer)
    ...
    while (buffer.remaining() > 0) {
      ...
      int bytesWritten =
          future.get(TCPUtils.DEFAULT_SOCKET_TIMEOUT_MS, TimeUnit.MILLISECONDS);
      if (bytesWritten == 0) {
