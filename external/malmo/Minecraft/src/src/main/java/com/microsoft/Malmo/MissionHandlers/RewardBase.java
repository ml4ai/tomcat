...
public class RewardBase extends HandlerBase implements IRewardProducer {
  ...
  protected float
  adjustAndDistributeReward(float reward, int dimension, String distribution) {
    ...
    if (ind != parties.size()) {
      ...
      {
        System.out.println(
            "ERROR: malformed argument for distribution of reward - " + us);
      ...
      scaled_reward = 0; // There's a distribution, but we're not included in it
                         // - we get nothing.
    // Now broadcast the reward to the other clients (but don't make a map entry
    // for ourselves)
