...
public class MinecraftVanillaAIHandler {
    ...
    public static void freezeZombie(EntityZombie zombie) {
        for (Object task : zombie.tasks.taskEntries.toArray()) {
            ...
            if (ai instanceof EntityAIZombieAttack ||
                ai instanceof EntityAIMoveTowardsRestriction ||
    ...
    public static void freezeVillager(EntityVillager villager) {
        for (Object task : villager.tasks.taskEntries.toArray()) {
            ...
            if (ai instanceof EntityAIMoveIndoors ||
                ai instanceof EntityAIOpenDoor ||
                ai instanceof EntityAIMoveTowardsRestriction ||
                ai instanceof EntityAIFollowGolem ||
                ai instanceof EntityAIWanderAvoidWater) {
    ...
    public static void unfreezeZombie(EntityZombie zombie) {
        ...
        zombie.tasks.addTask(5,
                             new EntityAIMoveTowardsRestriction(zombie, 1.0D));
        zombie.tasks.addTask(7, new EntityAIWanderAvoidWater(zombie, 1.0D));
        zombie.tasks.addTask(
            6, new EntityAIMoveThroughVillage(zombie, 1.0D, false));
        zombie.targetTasks.addTask(
            2,
            new EntityAINearestAttackableTarget<EntityPlayer>(
                zombie, EntityPlayer.class, true));
        zombie.targetTasks.addTask(
            3,
            new EntityAINearestAttackableTarget<EntityVillager>(
                zombie, EntityVillager.class, false));
        zombie.targetTasks.addTask(
            3,
            new EntityAINearestAttackableTarget<EntityIronGolem>(
                zombie, EntityIronGolem.class, true));
    }
    ...
    public static void unfreezeSkeleton(EntitySkeleton skeleton) {
        ...
        skeleton.targetTasks.addTask(
            2,
            new EntityAINearestAttackableTarget<EntityPlayer>(
                skeleton, EntityPlayer.class, true));
        skeleton.targetTasks.addTask(
            3,
            new EntityAINearestAttackableTarget<EntityIronGolem>(
                skeleton, EntityIronGolem.class, true));
    }
    ...
    public static void unfreezeVillager(EntityVillager villager) {
        ...
        villager.tasks.addTask(
            5, new EntityAIMoveTowardsRestriction(villager, 0.6D));
