...
public class TomcatClientServerHandler {
	 ...
	 */
	public static void sendMessageToAllClients(TomcatMessaging.TomcatMessage message, boolean expectReply) {							
	 ...
	 * @param message - Message to be sent	 
	 */
	public static void sendMessageToClient(EntityPlayerMP player, TomcatMessaging.TomcatMessage message, boolean expectReply) {
		if ((!isTomcat(player) || !isMultiplayer()) && !pendingPlayers.contains(player.getName())) {
			if (expectReply) {
	 ...
	 * @param message - Message to be sent	 
	 */
	public static void sendMessageToServer(TomcatMessaging.TomcatMessage message) {		
	...
	public static boolean isMultiplayer() {
		return MinecraftServerHelper.getServer().getPlayerList().getPlayers().size() > 1;
	}


	public static void handleMessageBackFromClient(EntityPlayerMP player, TomcatMessaging.TomcatMessage message) {
		pendingPlayers.remove(player.getName());
		MalmoMod.instance.getServer().getTomcatServerMission().handleMessageFromClient(player, message);		
	}
	...
	public static void handleMessageFromServer(TomcatMessaging.TomcatMessage message) {
		if (message.getMessageType() == TomcatMessageType.INIT_MISSION) {
  		  ClientMission clientMission = MissionFactory.createClient(message.getMessageData().getMissionID().ordinal());
