...
public class TutorialMission extends Mission {
	...
	protected void updateScene(World world) {
		...
		/* viewTime is in seconds. The player stays in each view mode for 3 seconds.
		 20 Minecraft ticks equal 1 real second. viewTime is incremented by 0.05 till 60 such ticks (3 seconds)d
		if (this.viewTime < 3.05) {
			this.changePlayerPerspective();
		}
		//System.out.println("===========>Player's position: " + MinecraftServerHelper.getFirstPlayer().getPosition().toString());d
		 */
	}
