...
public class ZombieMissionInitializer extends MissionInitializer {
	...
	private static double PROB_SKELETON = 0.25; // Probability of spawning a skeleton instead of a zombie
	 ...
	 */
	public ZombieMissionInitializer(DIFFICULTY levelOfDifficulty, UUID[] villagersIds, DrawingHandler drawingHandler) {
	...
	private void spawnEntities(World world) {
		try {
			...
			ArrayList<Building> availableSingleRoomBuildings = new ArrayList<Building>(singleRoomBuildings);
			ArrayList<Building> availableMultiRoomBuildings = new ArrayList<Building>(multiRoomBuildings);
			ArrayList<Building> remainingAvailableBuildings = new ArrayList<Building>();
			...
			for (int i = 0; i < this.villagersIds.length; i++) {
				...
				if ((random.nextDouble() < this.probSingleRoomBuilding && !availableSingleRoomBuildings.isEmpty()) ||
					...
					if (random.nextDouble() < this.probMainRoom) {
						...
						if (building.getNumberOfRooms() > 2) {
							room = random.nextInt(building.getNumberOfRooms() - 2) + 1;
						} else {
							room = 1;
						}
					}
				...
				this.spawnVillager(drawing, this.villagersIds[i], roomCoordinates);				
			}
			...
			for (Building building : remainingAvailableBuildings) {
				Iterator<BlockPos> roomsCoordinates = building.getUnoccupiedRooms();
				while(roomsCoordinates.hasNext()) {
					BlockPos coordinates = roomsCoordinates.next();

					this.spawnEnemy(drawing, coordinates);

					// Create another enemy in the same room displaced by one block in the x direction
	...
	private void spawnVillager(Drawing drawing, UUID id, BlockPos coordinates) {
		...
		TomcatEntity villager = new TomcatEntity(id, x, y, z, EntityTypes.VILLAGER);
	...
	private void equipEnemies(World world) {
		Random random = new Random();
		List<EntityMob> enemies = world.getEntities(EntityMob.class, new Predicate<EntityMob>() {		
			@Override
