...
public class DrawingHandler {
    ...
    public void draw(World world, Drawing drawing) throws Exception {
        ArrayList<JAXBElement<DrawObjectType>> jaxbeElements =
            this.getJAXBElements(drawing);
     ...
     * @return
     */
    private ArrayList<JAXBElement<DrawObjectType>>
    getJAXBElements(Drawing drawing) {
        Iterator<CompositeDrawingObject> objects = drawing.getObjects();
        Iterator<DrawObjectType> malmoDrawObjects;
        ArrayList<JAXBElement<DrawObjectType>> jaxbeObjects =
            new ArrayList<JAXBElement<DrawObjectType>>();
     ...
     */
    public void drawCountdown(long remainingSeconds,
                              long remainingSecondsAlert) {
        if (remainingSeconds >= 0) {
            TextComponentString text = new TextComponentString(
                "Mission ends in " +
                Converter.secondsToString(remainingSeconds, false) + "...");
