...
public abstract class Mission implements FeedbackListener, PhaseListener {
	...
	public void update(World world) {
		// These two initializations need to be here because the clients are not yet ready in the init method in a 
		// multiplayer setting
	...
	/**
	 * Freeze entities to create an effect of pausing the world in a multiplayer game
	 */
	private void freezeWorld(World world) {
		if (!this.worldFrozen) {
			for (EntityLiving entity : world.getEntities(EntityLiving.class, EntitySelectors.IS_ALIVE)) {
	...
	private void askClientsToUpdateCountdown(int remainingSeconds) {
		...
		TomcatMessaging.TomcatMessage message = new TomcatMessage(TomcatMessageType.UPDATE_COUNTDOWN, messageData);
	...
	private void initMissionOnClients() {
		if (!this.missionSentToClients) {
			for (EntityPlayerMP player : MinecraftServerHelper.getPlayers()) {
				...
				TomcatMessaging.TomcatMessage message = new TomcatMessage(TomcatMessageType.INIT_MISSION, messageData);
	...
	private void showSelfReportScreen(World world) {
		if (this.hasSelfReport()) {
			long elapsedTime =
					Converter.getElapsedTimeInSeconds(world, this.initialWorldTime + this.pausedTime);
			...
			if (elapsedTime % this.selfReportPromptTimeInSeconds == 0 &&
				...
				for (EntityPlayerMP player : MinecraftServerHelper.getPlayers()) {
					...
					TomcatMessage message = new TomcatMessage(TomcatMessageType.SHOW_SELF_REPORT, new TomcatMessageData(content));
	 ...
	 * @return
	 */
	protected abstract SelfReportContent getSelfReportContent(EntityPlayerMP player, World world);
	 ...
	 */
	public void handleMessageFromClient(EntityPlayerMP player, TomcatMessage message) {
		...
		case DISPLAY_INSTRUCTIONS:
			// TODO: fix this later so a single player can open the instructions without affect the others
	...
	private void unpauseMission() {
		...
		this.missionPaused = false;
		this.pausedTime += (MinecraftServerHelper.getServer().getEntityWorld().getTotalWorldTime() - this.worldTimeOnPause);
	}
	...
	public void pauseMission() {
		...
		this.worldTimeOnPause = MinecraftServerHelper.getServer().getEntityWorld().getTotalWorldTime(); 
	}
	
	/**
	 * Dismisses the message screen that informs the player he should wait for the others 
	 */
	...
	/**
	 * When a player tries to dismiss a screen but others are still with their screens prompted,
	 * show a message a screen saying the player he smust wait for the others to continue the mission.
	 * @param player
	 */
	private void showWaitingForOthersScreen(EntityPlayerMP player) {
		TomcatMessageData messageData = new TomcatMessageData();
	    messageData.setMissionPhaseMessage("Waiting for other players' confirmation.");
