...
public class RichContentScreen extends GUIScreenUndismissableOnEscapeKey {
   ...
   * @param dismissible - Indicates whether there is a button to close the
   * window
   * @param closeButtonLabel - Label of the button which closes the screen
   */
  public RichContentScreen(RichContent content,
  ...
  protected void drawButtons() {
    ...
    this.buttonPrevious = new GuiButton(ButtonCode.PREVIOUS.ordinal(),
                                        x,
                                        y,
                                        BUTTON_WIDTH,
                                        BUTTON_HEIGHT,
    ...
    this.closeButton = new GuiButton(ButtonCode.CLOSE.ordinal(),
                                     x,
                                     y,
                                     BUTTON_WIDTH,
                                     BUTTON_HEIGHT,
    ...
    x = x + BUTTON_WIDTH + spaceBetweenButtons;
    this.buttonNext = new GuiButton(
        ButtonCode.NEXT.ordinal(), x, y, BUTTON_WIDTH, BUTTON_HEIGHT, "Next");
  ...
  private void drawTextFromPage(RichContentPage page) {
    ...
    GuiLabel guiLabel = new UnshadowedLabel(
        this.fontRendererObj, 1, x, y, textWidth, textHeight, FOREGORUND_COLOR);
  ...
  private void drawImagesFromPage(RichContentPage page) {
    ...
    while (images.hasNext()) {
      ...
      this.drawTexturedModalRect(x,
                                 y + (maxImageHeight - image.getHeight()) / 2,
                                 image.getOriginXInTexture(),
                                 image.getOriginYInTexture(),
                                 image.getWidth(),
                                 image.getHeight());
   ...
   */
  private void drawImageCaption(RichContentImage image,
                                int imageX,
                                int imageY,
                                int maxImageHeightOnPage) {
  ...
  protected void actionPerformed(GuiButton guiButton) {
    ...
    case NEXT:
      this.currentPage =
          Math.min(this.content.getNumberOfPages() - 1, this.currentPage + 1);
      this.handleButtonsActivation();
      break;
