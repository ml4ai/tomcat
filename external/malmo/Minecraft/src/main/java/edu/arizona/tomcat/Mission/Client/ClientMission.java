...
public abstract class ClientMission implements ScreenListener {
	...
	public void handleMessageFromServer(TomcatMessage message) {
		...
		case UPDATE_COUNTDOWN:
			...
			long remainingSecondsAlert = message.getMessageData().getRemainingSecondsAlert();
			DrawingHandler.getInstance().drawCountdown(remainingSeconds, remainingSecondsAlert);
	...
	private void handleNextSelfReportScreen() {
		...
		case SHOWING_SCREEN_AFTER_QUESTIONS:
			...
			TomcatMessage message = new TomcatMessage(TomcatMessageType.SELF_REPORT_ANSWERED, data);
	...
	public void screenDismissed(GuiScreen screen) {
		if (this.selfReportPhase == SelfReportPhase.NOT_PROCESSING_SELF_REPORT) {
			TomcatMessage message = new TomcatMessage(TomcatMessageType.OPEN_SCREEN_DISMISSED);
			TomcatClientServerHandler.sendMessageToServer(message);
		}
		else {
			this.handleNextSelfReportScreen();
