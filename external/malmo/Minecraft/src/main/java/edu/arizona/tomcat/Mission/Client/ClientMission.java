...
public abstract class ClientMission implements ScreenListener {
	...
	private enum SelfReportPhase {SHOWING_SCREEN_BEFORE_QUESTIONS, PRESENTING_QUESTIONS, 
		SHOWING_SCREEN_AFTER_QUESTIONS, NOT_PROCESSING_SELF_REPORT};
	...
	private void handleNextSelfReportScreen() {
		...
		case NOT_PROCESSING_SELF_REPORT:
			...
			if (this.currentSelfReport.getRichContentBeforeQuestions() != null) {				
				RichContentScreen selfReportInitialRichContentScreen = new RichContentScreen(this.currentSelfReport.
						getRichContentBeforeQuestions(), true, true, "Ok");
				selfReportInitialRichContentScreen.addListener(this);
				Minecraft.getMinecraft().displayGuiScreen(selfReportInitialRichContentScreen);
				break;
			} else {
				this.handleNextSelfReportScreen();
			}
		case SHOWING_SCREEN_BEFORE_QUESTIONS:
			...
			SelfReportScreen selfReportScreen = new SelfReportScreen(this.currentSelfReport, true);
		...
		case PRESENTING_QUESTIONS:
			...
			if (this.currentSelfReport.getRichContentAfterQuestions() != null) {
				RichContentScreen selfReportFinalRichContentScreen = new RichContentScreen(this.currentSelfReport.
						getRichContentAfterQuestions(), true, true, "Ok");
				selfReportFinalRichContentScreen.addListener(this);
				Minecraft.getMinecraft().displayGuiScreen(selfReportFinalRichContentScreen);
			} else {
				this.handleNextSelfReportScreen();
			}
			break;
