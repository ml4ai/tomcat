...
public class RewardForStructureCopyingImplementation
    ...
    public void getReward(MissionInit missionInit,
                          MultidimensionalReward multidimReward) {
        ...
        if (this.rewardDensity == RewardDensityForBuildAndBreak.MISSION_END) {
            ...
            if (multidimReward.isFinalReward() && this.reward != 0) {
                float adjusted_reward = adjustAndDistributeReward(
                    this.reward,
                    this.dimension,
                    this.rscparams.getRewardDistribution());
    ...
    public void prepare(MissionInit missionInit) {
        ...
        MalmoMod.MalmoMessageHandler.registerForMessage(
            this, MalmoMessageType.SERVER_BUILDBATTLEREWARD);

        if (this.rscparams.getAddQuitProducer() != null) {
            // In order to trigger the end of the mission, we need to hook into
            // the quit handlers.
            MissionBehaviour mb = parentBehaviour();
            final String quitDescription =
            ...
            mb.addQuitProducer(new IWantToQuit() {
                ...
                public boolean doIWantToQuit(MissionInit missionInit) {
                    return RewardForStructureCopyingImplementation.this
                        .structureHasBeenCompleted;
                }
    ...
    public void onMessage(MalmoMessageType messageType,
        ...
        if (messageType == MalmoMessageType.SERVER_BUILDBATTLEREWARD &&
            data != null) {
            String strCompleted = data.get("completed");
            String strReward = data.get("reward");
            Boolean completed = strCompleted != null
                                    ? Boolean.valueOf(strCompleted)
                                    : Boolean.FALSE;
            ...
            synchronized (this) {
                if (completed == Boolean.TRUE) {
                    ...
                    this.reward +=
                        this.rscparams.getRewardForCompletion().floatValue();
                }
                if (reward != null) {
                    this.reward +=
                        reward * this.rscparams.getRewardScale().floatValue();
                }
            }
        }
    }
