...
public class RewardBase extends HandlerBase implements IRewardProducer {
    ...
    protected MultidimensionalReward cachedRewards =
        new MultidimensionalReward();

    public String getAgentName() { return this.agentName; }

    protected float adjustAndDistributeReward(float reward,
                                              int dimension,
                                              String distribution) {
        ...
        if (ind != parties.size()) {
            ...
            {
                System.out.println(
                    "ERROR: malformed argument for distribution of reward - " +
                    us);
            ...
            scaled_reward =
                0; // There's a distribution, but we're not included in it
                   // - we get nothing.
        // Now broadcast the reward to the other clients (but don't make a map
        // entry for ourselves)
        ...
        MalmoMod.network.sendToServer(new MalmoMod.MalmoMessage(
            MalmoMessageType.CLIENT_SHARE_REWARD, 0, data));
    ...
    protected void addCachedReward(MultidimensionalReward reward) {
        synchronized (this.cachedRewards) { this.cachedRewards.add(reward); }
    }

    protected void
    addAndShareCachedReward(int dimension, float reward, String distribution) {
        float adjusted_reward =
            adjustAndDistributeReward(reward, dimension, distribution);
