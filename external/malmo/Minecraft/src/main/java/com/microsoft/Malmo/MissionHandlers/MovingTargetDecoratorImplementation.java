...
public class MovingTargetDecoratorImplementation
    ...
    private ArrayDeque<IBlockState> originalPath =
        new ArrayDeque<IBlockState>();
    ...
    public boolean parseParameters(Object params) {
        ...
        this.blockType = (targetBlock != null)
                             ? new XMLBlockState(targetBlock.getType(),
                                                 targetBlock.getColour(),
                                                 targetBlock.getFace(),
                                                 targetBlock.getVariant())
                             : null;
        ...
        xPos = Math.min(this.arenaBounds.getMax().getX(),
                        Math.max(this.arenaBounds.getMin().getX(), xPos));
        yPos = Math.min(this.arenaBounds.getMax().getY(),
                        Math.max(this.arenaBounds.getMin().getY(), yPos));
        zPos = Math.min(this.arenaBounds.getMax().getZ(),
                        Math.max(this.arenaBounds.getMin().getZ(), zPos));
        this.startPos = new BlockPos(xPos, yPos, zPos);
        if (this.targetParams.getUpdateSpeed() == null ||
            this.targetParams.getUpdateSpeed().equals("turnbased")) {
            this.mustWaitTurn = true;
        }
        else {
            this.speedInTicks =
                Integer.parseInt(this.targetParams.getUpdateSpeed());
        }
        createRNG();
        return true;
    }
    ...
    private boolean pinchedByPlayer(World world) {
        for (BlockPos bp : this.path) {
            ...
            // AxisAlignedBB aabb = b.getCollisionBoundingBox(world, bp,
            // b.getDefaultState()); aabb.expand(0, 1, 0);
            BlockPos bp2 =
                new BlockPos(bp.getX() + 1, bp.getY() + 2, bp.getZ() + 1);
            AxisAlignedBB aabb = new AxisAlignedBB(bp, bp2);
            List<Entity> entities =
                world.getEntitiesWithinAABBExcludingEntity(null, aabb);
    ...
    public void update(World world) {
        ...
        if (!pinchedByPlayer(world)) {
            ...
            ArrayList<BlockPos> possibleMovesForward =
                new ArrayList<BlockPos>();
            ArrayList<BlockPos> possibleMovesBackward =
                new ArrayList<BlockPos>();
            for (int x = -1; x <= 1; x++) {
                for (int z = -1; z <= 1; z++) {
                    if (z != 0 && x != 0)
                    ...
                    BlockPos candidateHeadPos = new BlockPos(
                        posHead.getX() + x, posHead.getY(), posHead.getZ() + z);
                    BlockPos candidateTailPos = new BlockPos(
                        posTail.getX() + x, posTail.getY(), posTail.getZ() + z);
            ...
            if (possibleMovesBackward.isEmpty()) {
                ...
                candidates =
                    forwards ? possibleMovesForward : possibleMovesBackward;
            }
            if (!candidates.isEmpty()) {
                ...
                BlockPos newPos =
                    candidates.get(this.rng.nextInt(candidates.size()));
                if (forwards) {
                    // Add the new head:
                    ...
                    if (this.path.size() > this.pathSize) {
                        drawContext.setBlockState(
                            world,
                            posTail,
                            new XMLBlockState(this.originalPath.removeLast()));
                    ...
                    if (this.path.size() > this.pathSize) {
                        drawContext.setBlockState(
                            world,
                            posHead,
                            new XMLBlockState(this.originalPath.removeFirst()));
        ...
        if (this.mustWaitTurn) {
            ...
            MalmoMod.network.sendToServer(new MalmoMod.MalmoMessage(
                MalmoMessageType.CLIENT_TURN_TAKEN, 0, data));
        }
    }
    ...
    private boolean isValid(World world, BlockPos pos) {
        ...
        if (this.targetParams.isRequiresAirAbove() &&
            !world.isAirBlock(pos.up()))
            return false;
        // Check the current block is "permeable"...
        IBlockState block = world.getBlockState(pos);
        List<IProperty> extraProperties = new ArrayList<IProperty>();
        DrawBlock db = MinecraftTypeHelper.getDrawBlockFromBlockState(
            block, extraProperties);

        boolean typesMatch =
            this.targetParams.getPermeableBlocks().getType().isEmpty();
        ...
        if (db.getColour() != null) {
            boolean coloursMatch =
                this.targetParams.getPermeableBlocks().getColour().isEmpty();
            for (Colour col :
                 this.targetParams.getPermeableBlocks().getColour()) {
                if (db.getColour() == col) {
                    coloursMatch = true;
                    break;
                }
        ...
        if (db.getVariant() != null) {
            boolean variantsMatch =
                this.targetParams.getPermeableBlocks().getVariant().isEmpty();
            for (Variation var :
                 this.targetParams.getPermeableBlocks().getVariant()) {
    ...
    private void createRNG() {
        ...
        long seed = 0;
        if (this.targetParams.getSeed() == null ||
            this.targetParams.getSeed().equals("random"))
