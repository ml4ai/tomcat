...
public class TurnBasedCommandsImplementation
    ...
    public boolean parseParameters(Object params) {
        ...
        if (params instanceof TurnBasedCommands) {
            ...
            this.requestedPosition =
                tbcparams.getRequestedPosition().intValue();
            // Create our partner observation producer, and manually add it to
            // the mission behaviour:
            this.observationProducer =
                new ObservationFromTurnSchedulerImplementation();
            // this.parentBehaviour().addObservationProducer(this.observationProducer);
            // Now instantiate our child handlers. We can use a new
            // MissionBehaviour object to take care of this.
            List<Object> handlers =
                tbcparams.getTurnBasedApplicableCommandHandlers();
            if (!handlers.isEmpty()) {
                ...
                if (subHandlers.commandHandler != null) {
                    if (subHandlers.commandHandler instanceof HandlerBase)
                        ((HandlerBase)subHandlers.commandHandler)
                            .setParentBehaviour(this.parentBehaviour());
    ...
    onExecute(String verb, String parameter, MissionInit missionInit) {
        // Firstly, reject any command that doesn't match the single-use turn
        // key provided to the agent.
        if (!this.observationProducer.matchesKey(verb)) {
            // Incorrect key. For the moment, we just let this command
            // disappear. In the future, we should return an error to the agent.
            return false;
        }
        // Now pass the command on to our sub-handlers.
        // The key will have been stripped off as the first parameter;
        // we assume the parameter string consists of the *real* command, plus
        // parameters. CommandGroup.onExecute rejoins the verb and parameter
        // before passing to the children, so we can pass an empty verb, and the
