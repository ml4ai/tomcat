...
public class ObservationFromHumanImplementation
    ...
    private class MouseObserver
        implements MalmoModClient.MouseEventListener, KeyEventListener {
        @Override
    ...
    public void queueEvent(ObservationEvent event) {
        synchronized (this.events) { this.events.add(event); }
    }
    ...
    public void prepare(MissionInit missionInit) {
        ...
        if (!(mhelp instanceof MalmoModClient.MouseHook)) {
            System.out.println(
                "ERROR! MouseHook not installed - Malmo won't work correctly.");
            return;
        }
