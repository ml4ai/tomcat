...
public class TCPSocketChannel {
    ...
    /**
     * Create a TCPSocketChannel that is blocking but times out connects and
     * writes.
    ...
    public TCPSocketChannel(String address, int port, String logname) {
        ...
        try {
            ...
            Log(Level.SEVERE,
                "Failed to connectWithTimeout AsynchronousSocketChannel: " + e);
        }
            ...
            Log(Level.SEVERE,
                "AsynchronousSocketChannel connectWithTimeout timed out: " + e);
        }
    }
    ...
    private void connectWithTimeout() throws IOException, ExecutionException,
        ...
        Log(Level.INFO,
            "Attempting to open SocketChannel with InetSocketAddress: " +
                inetSocketAddress);
        this.channel = AsynchronousSocketChannel.open();
        Future<Void> connected = this.channel.connect(inetSocketAddress);
        connected.get(TCPUtils.DEFAULT_SOCKET_TIMEOUT_MS,
    ...
    /**
     * Send string over TCP to the specified address via the specified port,
     * including a header.
     *
    ...
    public boolean sendTCPBytes(byte[] bytes, int retries) {
        try {
            ...
            SysLog(Level.SEVERE,
                   "Failed to send TCP bytes" +
                       (retries > 0 ? " -- retrying " : "") + ": " + e);
            ...
            if (retries > 0) {
                try {
                    ...
                    SysLog(Level.SEVERE,
                           "Failed to reconnect: " + connectException);
                    return false;
                }
    ...
    public boolean sendTCPBytes(ByteBuffer[] srcbuffers, int length) {
        ...
        try {
            ...
            if (TCPUtils.isLogging()) {
                ...
                double rate = 1000.0 * 1000.0 * 1000.0 *
                              (double)(bytesWritten) /
                              (1024.0 * (double)(t2 - t1));
                Log(Level.INFO,
                    "Sent " + bytesWritten + " bytes at " + rate + " Kb/s");
            }
            else {
                write(buffers);
            }
    ...
    private ByteBuffer createHeader(int length) {
        ByteBuffer header =
            ByteBuffer.allocate(4).order(ByteOrder.BIG_ENDIAN).putInt(length);
        header.flip();
        return header;
    }

    private void safeWrite(ByteBuffer buffer)
        throws InterruptedException, TimeoutException, ExecutionException,
               IOException {
        while (buffer.remaining() > 0) {
            Future<Integer> future = this.channel.write(buffer);
            int bytesWritten = future.get(TCPUtils.DEFAULT_SOCKET_TIMEOUT_MS,
                                          TimeUnit.MILLISECONDS);
            if (bytesWritten == 0) {
    ...
    private long write(ByteBuffer[] buffers)
        throws InterruptedException, TimeoutException, ExecutionException,
               IOException {
        long bytesWritten = 0;
