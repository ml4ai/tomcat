...
public class TCPUtils {
    public enum SeverityLevel {
        ...
        SeverityLevel(String displayName, Level level) {
    ...
    static class UTCFormatter extends Formatter {
        ...
        private static final FastDateFormat DATE_FORMATTER =
            FastDateFormat.getInstance(dateformat, TimeZone.getTimeZone("UTC"));
        private static final String padding =
            dateformat +
            "00000"; // to pad the milliseconds up to 6 spaces - see below.
        ...
        public String format(LogRecord record) {
            ...
            // "000" padding is to match the greater precision available from
            // the C++ side, if combining logs.
            timestamp += padding.substring(timestamp.length());
            builder.append(timestamp)
                .append(" M ") // 'M' for 'Mod' - useful if combining logs with
                               // platform-side.
    ...
    private static Logger logger =
        Logger.getLogger("com.microsoft.Malmo.TCPUtils");
    ...
    public static void setLogging(SeverityLevel slevel) {
        ...
        if (logging == true && filehandler == null) {
            try {
                Date d = new Date();
                String filename =
                    "TCP" + DateFormatUtils.format(d, "yyyy-MM-dd HH-mm-ss") +
                    ".log";
                filehandler =
                    new FileHandler("logs" + File.separator + filename);
    ...
    private static String getIndented(String message) {
        return (currentIndentation == 0)
            ? message
            : StringUtils.repeat("    ", currentIndentation) + message;
    }
    ...
    public static void update(Configuration config) {
        ...
        String severityLevel = config.getString(
            "loggingSeverityLevel",
            MalmoMod.DIAGNOSTIC_CONFIGS,
            TCPUtils.loggingSeverityLevel.getDisplayName(),
            "Set the level of socket debugging information to be logged.",
            values);
        for (SeverityLevel level : SeverityLevel.values())
    ...
    private static void indent() { currentIndentation++; }

    private static void unindent() { currentIndentation--; }
    ...
    /**
     * Choose a port from the specified range - either sequentially, or at
     * random.
     *
     * @param minPort     minimum (inclusive) value for port.
     * @param maxPort     max (inclusive) possible port value.
     * @param random      true to allocate based on a random sample; false to
     * allocate sequentially, starting from minPort.
     * @return a ServerSocket.
     */
    public static ServerSocket
    getSocketInRange(int minPort, int maxPort, boolean random) {
        TCPUtils.Log(Level.INFO,
                     "Attempting to create a ServerSocket in range (" +
                         minPort + "-" + maxPort +
                         (random ? ") at random..." : ") sequentially..."));
        ServerSocket s = null;
        int port = minPort - 1;
