        ...
        for (int x = -1; x <= 1 && !canEscape; x++) {
          for (int z = -1; z <= 1 && !canEscape; z++) {
            ...
            BlockPos square = new BlockPos(
                entityPos.getX() + x, entityPos.getY(), entityPos.getZ() + z);
      ...
      for (EntityTypes et : mob.getType()) {
        ...
        for (Entity e : trappedEntities) {
          if (e.getName().equals(mobName)) {
            ...
            if (!mob.isGlobal()) {
              // If global flag is false, our player needs to be adjacent to the
              // mob in order to claim the reward.
              ...
              if (Math.abs(entityPos.getX() - playerPos.getX()) +
                      Math.abs(entityPos.getZ() - playerPos.getZ()) >
                  1)
                continue;
            }
            // If oneshot flag is true, only allow the reward from this mob to
            // be counted once.
            if (mob.isOneshot() && this.caughtEntities.contains(e))
              continue;
            // Can claim the reward.
            float adjusted_reward =
                adjustAndDistributeReward(mob.getReward().floatValue(),
