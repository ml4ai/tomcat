    ...
    static List<Entity> getCaughtEntities() {
        ...
        for (Object obj : entities) {
            ...
            if (obj instanceof Entity) {
                Entity e = (Entity)obj;
                BlockPos entityPos =
                    new BlockPos((int)e.posX, (int)e.posY, (int)e.posZ);
                ...
                for (int x = -1; x <= 1 && !canEscape; x++) {
                    for (int z = -1; z <= 1 && !canEscape; z++) {
                        if (Math.abs(x) == Math.abs(z))
                            continue; // Only consider the n/s/e/w blocks -
                                      // ignore diagonals.
                        BlockPos square = new BlockPos(entityPos.getX() + x,
                                                       entityPos.getY(),
                                                       entityPos.getZ() + z);
                        if (world.isAirBlock(square) &&
                            !entityPositions.contains(square))
                            canEscape = true;
                    }
                }
                if (!canEscape) {
            ...
            for (EntityTypes et : mob.getType()) {
                ...
                for (Entity e : trappedEntities) {
                    if (e.getName().equals(mobName)) {
                        ...
                        if (!mob.isGlobal()) {
                            // If global flag is false, our player needs to be
                            // adjacent to the mob in order to claim the reward.
                            BlockPos entityPos =
                                new BlockPos(e.posX, e.posY, e.posZ);
                            EntityPlayerSP player =
                                Minecraft.getMinecraft().player;
                            BlockPos playerPos = new BlockPos(
                                player.posX, player.posY, player.posZ);
                            if (Math.abs(entityPos.getX() - playerPos.getX()) +
                                    Math.abs(entityPos.getZ() -
                                             playerPos.getZ()) >
                                1)
                                continue;
                        }
                        // If oneshot flag is true, only allow the reward from
                        // this mob to be counted once.
                        if (mob.isOneshot() && this.caughtEntities.contains(e))
                            continue;
                        // Can claim the reward.
                        float adjusted_reward = adjustAndDistributeReward(
                            mob.getReward().floatValue(),
                            this.rcmparams.getDimension(),
                            mob.getDistribution());
                        reward.add(this.rcmparams.getDimension(),
                                   adjusted_reward);
                    }
                }
            }
