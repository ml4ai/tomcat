...
public class RewardForDamagingEntityImplementation
    ...
    public void getReward(MissionInit missionInit,
        ...
        synchronized (this.damages) {
            for (MobWithReward mob : this.damages.keySet()) {
                ...
                float damage_reward =
                    damage_amount * mob.getReward().floatValue();
                float adjusted_reward =
                    adjustAndDistributeReward(damage_reward,
    ...
    public void onLivingAttackEvent(LivingAttackEvent event) {
        if (event.getEntity() == null ||
            event.getSource().getEntity() != Minecraft.getMinecraft().player)
            return;
        synchronized (this.damages) {
                ...
                for (EntityTypes et : mob.getType()) {
                    ...
                    if (event.getEntity().getName().equals(mobName)) {
                        if (this.damages.containsKey(mob))
                            this.damages.put(
                                mob, this.damages.get(mob) + event.getAmount());
                        else
