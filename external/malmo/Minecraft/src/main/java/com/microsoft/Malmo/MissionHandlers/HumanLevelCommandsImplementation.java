...
public class HumanLevelCommandsImplementation extends CommandGroup {
    ...
    TimeHelper.TickRateMonitor clientTickMonitor =
        new TimeHelper.TickRateMonitor();
    TimeHelper.TickRateMonitor renderTickMonitor =
        new TimeHelper.TickRateMonitor();
        ...
        if (verb.equalsIgnoreCase(HumanLevelCommand.MOVE_MOUSE.value())) {
            if (parameter != null && parameter.length() != 0) {
                ...
                if (params.length != 2 && params.length != 3) {
                    System.out.println("Malformed parameter string (" +
                                       parameter +
                ...
                if (x != 0 || y != 0) {
                    ...
                    if (this.isOverriding())
                        mc.player.turn(this.targetYawDelta,
                                       this.targetPitchDelta);
                    int renderTicksPerClientTick =
                        this.clientTickMonitor.getEventsPerSecond() >= 1
                            ? (int)Math.ceil(
                                  this.renderTickMonitor.getEventsPerSecond() /
                                  this.clientTickMonitor.getEventsPerSecond())
                            : 0;
                    renderTicksPerClientTick =
                        Math.max(1, renderTicksPerClientTick);
                    this.targetYawDelta = f2;
                    this.targetPitchDelta = f3;
                    this.targetYawDeltaDelta =
                        f2 / (float)renderTicksPerClientTick;
                    this.targetPitchDeltaDelta =
                        f3 / (float)renderTicksPerClientTick;
                    // System.out.println("Changing over " +
                    // renderTicksPerClientTick + " render ticks.");
                }
                if (z != 0) {
                    ...
                    if (!Minecraft.getMinecraft().player.isSpectator() &&
                        ...
                        Minecraft.getMinecraft()
                            .player.inventory.changeCurrentItem(z);
                    }
                }
                return true;
            }
    ...
    public void onRenderTick(TickEvent.RenderTickEvent ev) {
        if (ev.phase == Phase.START && this.isOverriding()) {
            ...
            if (this.isOverriding()) {
                ...
                if (player != null) {
                    if (this.targetPitchDelta != 0 ||
                        this.targetYawDelta != 0) {
                        player.turn(this.targetYawDeltaDelta,
                                    this.targetPitchDeltaDelta);
                        this.targetYawDelta -= this.targetYawDeltaDelta;
                        this.targetPitchDelta -= this.targetPitchDeltaDelta;
                        if (this.targetYawDelta / this.targetYawDeltaDelta <
                            1.0)
                            this.targetYawDeltaDelta = this.targetYawDelta;
                        if (this.targetPitchDelta / this.targetPitchDeltaDelta <
                            1.0)
