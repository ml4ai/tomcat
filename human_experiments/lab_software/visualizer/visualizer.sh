...
sync_images_pos() {
  ...
  sync_file_nam="lion_file_name";
  get_sync_vars;
  src_si="${sync_file_dtt}";
  dti_si="${sync_file_dti}";

  sync_file_nam="nam_si";
  get_sync_vars;

  if ( ${show_sync_work} ); then
    printf "\n${Yellow}Lions Current File Name: ${BPurple}${lion_file_name}    ${Yellow}Lions Current Pos: ${BPurple}${lion_i}${NC}\n";
    printf "${Yellow}Syncing: ${BPurple}$3   ${Yellow}Current Pos: ${BPurple}${pos_si}   ${Yellow}Images Cnt: ${BPurple}${cnt_si}${NC}\n";
    printf "${Yellow}Search Array: ${BPurple}${arr_si}   ${Yellow}Search Str: ${BPurple}${src_si}${NC}\n";
    printf "${Yellow}$3 First File Name: ${BPurple}${nam_si}   ${Yellow}$3 First File dti: ${BPurple}${sync_file_dti}   ${Yellow}Lions dti: ${BPurple}${dti_si}${NC}\n";
  fi

  if (( ${dti_si#0} < ${sync_file_dti#0} )); then eval $4="\"(waiting - images came later)\""; eval $1=0; return 1; fi;
  res_si=$(( $(printf "%s\n" "${!arr_si}" | sed -n '/.*'${src_si}'.*$/{=;q}') - 1 ));
  if (( ${res_si} >= 0 )); then
    eval $1=${res_si};
    eval $4="\"(synced)\"";
    if ( ${show_sync_work} ); then printf "${Green}Found: ${BPurple}${res_si}${NC}\n\n\n"; fi;
  else
      eval $4="\" \"";
    if ( ${show_sync_work} ); then printf "${Red}Not-Found!${NC}\n\n\n"; fi;
  fi
}
...
jump_images_pos(){
  # jump_images_pos "lion_i" "lion_i_cnt" "lion_files" "jump_sec";
  pos_jp="${!1}";             # Calling - i number
  pos_jp=$(( $pos_jp - 1 ));  # Correct for array index number
  cnt_jp="${!2}";             # Calling - count of files names in array
  lst_jp=$(( $cnt_jp - 1 ));  # Calling - last index number in array
  arr_jp="$3[@]";             # Calling - array (all)
  sec_jp="$4";                # Calling - number of seconds to jump (can be positive or negative)

  far_jp="$3[0]";             # Calling - array (0 index)
  car_jp="$3[${pos_jp}]";     # Calling - array (current i index)
  lar_jp="$3[${lst_jp}]";     # Calling - array (current last index)

  ffn_jp="${!far_jp}";        # Calling - first file name in array
  cfn_jp="${!car_jp}";        # Calling - current pos file name in array
  lfn_jp="${!lar_jp}";        # Calling - last file name in array

  sync_file_nam="ffn_jp";
  get_sync_vars;
  fds_jp="${sync_file_dtt}"   # First File Name DateTime String (yyyy-mm-dd_hh-mm-ss)
  fdt_jp=$(date -d "${fds_jp: -19: 10} ${fds_jp: -8: 2}:${fds_jp: -5: 2}:${fds_jp: -2: 2}" "+%Y-%m-%d %H:%M:%S") # Convert to DateTime Type
  fdi_jp=$(date -d "${fdt_jp}" "+%Y%m%d%H%M%S") # Convert to DateTime Int Type (yyyymmddhhnnss)

  sync_file_nam="cfn_jp";
  get_sync_vars;
  cds_jp="${sync_file_dtt}"   # Current File Name DateTime String (yyyy-mm-dd_hh-mm-ss)
  cdt_jp=$(date -d "${cds_jp: -19: 10} ${cds_jp: -8: 2}:${cds_jp: -5: 2}:${cds_jp: -2: 2}" "+%Y-%m-%d %H:%M:%S") # Convert to DateTime Type
  cdi_jp=$(date -d "${cdt_jp}" "+%Y%m%d%H%M%S") # Convert to DateTime Int Type (yyyymmddhhnnss)

  sync_file_nam="lfn_jp";
  get_sync_vars;
  lds_jp="${sync_file_dtt}"   # Last File Name DateTime String (yyyy-mm-dd_hh-mm-ss)
  ldt_jp=$(date -d "${lds_jp: -19: 10} ${lds_jp: -8: 2}:${lds_jp: -5: 2}:${lds_jp: -2: 2}" "+%Y-%m-%d %H:%M:%S") # Convert to DateTime Type
  ldi_jp=$(date -d "${ldt_jp}" "+%Y%m%d%H%M%S") # Convert to DateTime Int Type (yyyymmddhhnnss)

  # Calc New DateTime based on current File Name DateTime string offset by calling seconds (yyyy-mm-dd hh:mm:ss)
  new_jp="$( date -d "$(date -Iseconds -d  "${cdt_jp}") 0 hours 0 minutes ${sec_jp} seconds" "+%Y-%m-%d %H:%M:%S")"
  nwi_jp=$(date -d "${new_jp}" "+%Y%m%d%H%M%S") # Convert to DateTime Int Type (yyyymmddhhnnss)

  # if [[ ${new_jp} -le ${fdt_jp} ]]; then eval $1=0; return 1; fi;
  # if [[ ${new_jp} -ge ${ldt_jp} ]]; then eval $1=${lst_jp}; return 1; fi;
  nsi_jp=999999;
  if [[ ${nwi_jp#0} -le ${fdi_jp#0} ]]; then nsi_jp=0; fi;
  if [[ ${nwi_jp#0} -ge ${ldi_jp#0} ]]; then nsi_jp=${lst_jp}; fi;
  srs_jp="$(date -d "${new_jp}" "+%Y-%m-%d_%H-%M-%S")" # Convert DateTime String for searching File Names (yyyy-mm-dd_hh-mm-ss)

  if [[ ${nsi_jp#0} = 999999 ]]; then
    res_jp=$(( $(printf "%s\n" "${!arr_jp}" | sed -n '/.*'${srs_jp}'.*$/{=;q}') - 1 ));
    if (( ${res_jp#0} >= 0 )); then
      nsi_jp=${res_jp};
    fi
  fi

  if [[ ${nsi_jp#0} != 999999 ]]; then eval $1=${nsi_jp}; fi;

  if ( ${show_jump_work} ); then
    printf "\n${Green}Search Array: ${BPurple}${arr_jp}    ${Yellow}Current Pos: ${BPurple}${pos_jp}    ${Yellow}File Count: ${BPurple}${cnt_jp}${NC}\n";
    printf "${Yellow}Current File Name: ${BPurple}${cfn_jp} - ${Yellow}DT Str: ${BPurple}${cds_jp}${NC}\n";
    printf "${Yellow}  First File Name: ${BPurple}${ffn_jp}   - ${Yellow}DT Str: ${BPurple}${fds_jp}${NC}\n";
    printf "${Yellow}   Last File Name: ${BPurple}${lfn_jp} - ${Yellow}DT Str: ${BPurple}${lds_jp}${NC}\n";
    printf "${Yellow}Current Date Time: ${BPurple}${cdt_jp} - ${Yellow}Int: ${BPurple}${cdi_jp}${NC}\n";
    printf "${Yellow}  First Date Time: ${BPurple}${fdt_jp} - ${Yellow}Int: ${BPurple}${fdi_jp}${NC}\n";
    printf "${Yellow}   Last Date Time: ${BPurple}${ldt_jp} - ${Yellow}Int: ${BPurple}${ldi_jp}${NC}\n";
    printf "${Yellow}    New Date Time: ${BPurple}${new_jp} - ${Yellow}Int: ${BPurple}${nwi_jp}${NC}\n";
    printf "${Yellow}    DT Search Str: ${BPurple}${srs_jp}        ${Yellow}New si: ${BGreen}${nsi_jp}${NC}\n\n";
  fi
}


do_jump_images(){
  # "integer number of seconds string" (6 char - can be positive or negative s#####)
  sst_js="${1: -5: 5}";
  sin_js="${1: -6: 1}";
  sit_js=$((10#$sst_js + 0));
  if [[ "${sin_js}" == "-" ]]; then
  ...
  if ( ${show_jump_work} ); then
    printf "\n${BBlue}Jump Seconds In Arg : ${BRed}$1${NC}\n";
    printf "${BBlue}sst_js: ${BRed}${sst_js}${NC}\n";
    printf "${BBlue}sin_js: ${BRed}${sin_js}${NC}\n";
    printf "${BBlue}sit_js: ${BRed}${sit_js}${NC}\n";
  fi
  printf "${BBlue}Jump Images: ${col_js}${sec_js} Seconds${NC}\n";

  jump_images_pos "lion_i" "lion_i_cnt" "lion_files" "${sec_js}";
  jump_images_pos "tiger_i" "tiger_i_cnt" "tiger_files" "${sec_js}";
  jump_images_pos "leopard_i" "leopard_i_cnt" "leopard_files" "${sec_js}";
  jump_images_pos "lion_s" "lion_s_cnt" "lion_sfiles" "${sec_js}";
  jump_images_pos "tiger_s" "tiger_s_cnt" "tiger_sfiles" "${sec_js}";
  jump_images_pos "leopard_s" "leopard_s_cnt" "leopard_sfiles" "${sec_js}";
}
...
get_browser_command(){
  ...
  if [[ -f "${bfil}" ]]; then
    ...
    if [[ "${comm: 0: 5}" == "jump:" ]]; then do_jump_images "${comm: -6: 6}"; fi
