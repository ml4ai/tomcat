...
def read_affective_task_timestamps_individual(
    block, finger_tapping_marker, input_path, output_path
):
    ...
    for i in range(0, len(block)):
        if block[i]["info"]["name"][0] == "AffectiveTask_tiger":
            ...
            full_path = os.path.join(
                output_path,
                exp_name,
                "baseline_tasks/affective/individual_"
                + sub_id_tiger
                + "_"
                + str(timestamp_tiger)
                + ".csv",
...
def read_affective_task_timestamps_team(
    block, AffectiveTask_individual_marker, input_path, output_path
):
