...
def read_finger_tapping_time(block, rest_state_marker, input_path, output_path):
    ...
    for i in range(0,len(block)):
        if block[i]['info']['name'][0] ==  'FingerTapping':
            ...
            markers[idx] = {"state":"finger_tapping",
                             "participant": None,
            ...
            full_path = os.path.join(output_path, exp_name, "baseline_tasks/finger_tapping/"+str(timestamp)+".csv")
            df.to_csv(full_path, sep=';')
            print(colored("[Status] Saving Finger Tapping task to ", "green", attrs=["bold"]),
