...
def read_ping_pong_timestamps(block, AffectiveTask_team_marker, input_path, output_path):
    ...
    for i in range(0,len(block)):
        if block[i]['info']['name'][0] == 'PingPong_competitive_0':
            ...
            PingPong_competitive_0_timestamp = int(PingPong_competitive_0['lsl_timestamp'].iloc[0])

            full_path = os.path.join(output_path, exp_name, "baseline_tasks/ping_pong/competitive_0_"+str(PingPong_competitive_0_timestamp)+".csv")
            PingPong_competitive_0.to_csv(full_path, sep=';')

            print(colored("[Status] Saving Ping Pong Competetive 0 to ", "green", attrs=["bold"]),
        ...
        if block[i]['info']['name'][0] == 'PingPong_competitive_1':
            ...
            PingPong_competitive_1_timestamp = int(PingPong_competitive_1['lsl_timestamp'].iloc[0])


            full_path = os.path.join(output_path, exp_name, "baseline_tasks/ping_pong/competitive_1_"+str(PingPong_competitive_1_timestamp)+".csv")
            PingPong_competitive_1.to_csv(full_path, sep=';')
            print(colored("[Status] Saving Ping Pong Competetive 1 to ", "green", attrs=["bold"]),
                colored(full_path, "blue"),
            )
        ...
        if block[i]['info']['name'][0] == 'PingPong_cooperative_0':
            ...
            PingPong_cooperative_0_timestamp = int(PingPong_cooperative_0['lsl_timestamp'].iloc[0])

            full_path = os.path.join(output_path, exp_name, "baseline_tasks/ping_pong/cooperative_0_"+str(PingPong_cooperative_0_timestamp)+".csv")
            PingPong_cooperative_0.to_csv(full_path, sep=';')
            print(colored("[Status] Saving Ping Pong Coorperative 0 to ", "green", attrs=["bold"]),
                colored(full_path, "blue"),
            )
