...
def read_rest_state_timestamps(block, input_path, output_path):
    ...
    for i in range(0,len(block)):
        if block[i]['info']['name'][0] ==  'RestState':
            ...
            full_path = os.path.join(output_path, exp_name, "baseline_tasks/rest_state/"+str(rest_state_timestamp)+".csv")
            df.to_csv(full_path, sep=';')
            print(colored("[Status] Saving Rest state to", "green", attrs=["bold"]), 
