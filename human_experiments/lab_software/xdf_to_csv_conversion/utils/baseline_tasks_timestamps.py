        ...
        if os.path.isdir(os.path.join(rootdir, x)):
            if "rest_state" in x:
                ...
                for csvfile in csvfiles:
                    df = pd.read_csv(
                        os.path.join(os.path.join(rootdir, x), csvfile), delimiter=";"
                    )
                    start_stop_time[idx] = {
                        "state": "rest_state",
                        "participant": None,
                ...
                for csvfile in sorted(csvfiles):
                    if "team_" in csvfile:  # if csv filename is team_*
                        df = pd.read_csv(
                            os.path.join(os.path.join(rootdir, x), csvfile),
                            delimiter=";",
                        )
                        start_stop_time[idx] = {
                ...
                for csvfile in sorted(csvfiles):
                    if "competitive_0" in csvfile:
                        ...
                        subject_id_pp = []
                        for sub_id in df.columns[idx:]:
                            subject_id_pp.append(sub_id)
                        subject_id_pp = {
                            x.removesuffix("_x").removesuffix("_y")
                            for x in subject_id_pp
                        }
                        # print(subject_id_pp)
