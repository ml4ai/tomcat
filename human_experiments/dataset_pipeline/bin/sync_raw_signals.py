...
from dataset_pipeline.common.config import LOG_DIR
from dataset_pipeline.common.constants import STATIONS
from dataset_pipeline.database.config import SQLALCHEMY_DATABASE_URI, engine, get_db
from dataset_pipeline.database.entity.base.group_session import GroupSession
from dataset_pipeline.derived.helper.factory import create_modality_helper
from dataset_pipeline.derived.main_clock import get_main_clock_timestamps
from dataset_pipeline.derived.helper.factory import MODALITIES
...
def sync_raw_signals_in_parallel(
    ...
    """
    db = next(get_db())
    # Every session is visited; the per-station has_saved_sync_data check below
    # skips work that is already saved. Prefiltering on "session has any rows"
    # (the previous behavior) made a partially-synced session permanently
    # unhealable: exp_2023_04_17_13 saved tiger, failed on leopard, and every
    # rerun then skipped the whole session, so the leopard pair stayed missing
    # through two releases.
    remaining_group_sessions = sorted(db.scalars(select(GroupSession.id)).all())
    db.close()
...
def sync_raw_signals_single_job(
    ...
    for group_session in group_sessions:
        ...
        if clock_timestamps is None or len(clock_timestamps) == 0:
            error(
                f"Could not determine clock timestamps for the group session ({group_session})."
            )
            continue

        info(
            f"There are {len(clock_timestamps)} ticks in the main clock from timestamp "
