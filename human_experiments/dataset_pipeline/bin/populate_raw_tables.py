...
from dataset_pipeline.database.config import SQLALCHEMY_DATABASE_URI
...
def populate_tables(tables_to_process):
    # Processors are imported lazily inside each branch so selecting a light entity (e.g.
    # `base`) does not import heavy optional dependencies pulled in by others (vocalics
    # needs torch/whisper via the `audio` extra). Only the selected entities' imports run.
    #
    # The order is important such that the signals have valid foreign keys to the base tables and
    # task tables.
    if "base" in tables_to_process:
        from dataset_pipeline.raw.process_base_tables import process_base_tables
        process_base_tables()
    if "rest_state" in tables_to_process:
        from dataset_pipeline.raw.process_rest_state_task_data import (
            process_rest_state_task_data,
        )
        process_rest_state_task_data()
    if "affective" in tables_to_process:
        from dataset_pipeline.raw.process_affective_task_data import (
            process_affective_task_data,
        )
        process_affective_task_data()
    if "finger_tapping" in tables_to_process:
        from dataset_pipeline.raw.process_finger_tapping_task_data import (
            process_finger_tapping_task_data,
        )
        process_finger_tapping_task_data()
    if "ping_pong_competitive" in tables_to_process:
        from dataset_pipeline.raw.process_ping_pong_competitive_data import (
            process_ping_pong_competitive_task_data,
        )
        process_ping_pong_competitive_task_data()
    if "ping_pong_cooperative" in tables_to_process:
        from dataset_pipeline.raw.process_ping_pong_cooperative_data import (
            process_ping_pong_cooperative_task_data,
        )
        process_ping_pong_cooperative_task_data()
    if "minecraft" in tables_to_process:
        from dataset_pipeline.raw.process_minecraft_data import process_minecraft_data
        process_minecraft_data()
    if "screen_capture" in tables_to_process:
        from dataset_pipeline.raw.process_screen_capture_data import (
            process_screen_capture_data,
        )
        process_screen_capture_data()
    if "vocalics" in tables_to_process:
        from dataset_pipeline.raw.process_vocalics import process_vocalics
        process_vocalics()
    if "fnirs" in tables_to_process:
        from dataset_pipeline.raw.process_fnirs_raw_data import process_fnirs_raw_data
        process_fnirs_raw_data()
    if "gaze" in tables_to_process:
        from dataset_pipeline.raw.process_gaze_raw_data import process_gaze_raw_data
        process_gaze_raw_data()
    if "eeg" in tables_to_process:
        from dataset_pipeline.raw.process_eeg_raw_data import process_eeg_raw_data
