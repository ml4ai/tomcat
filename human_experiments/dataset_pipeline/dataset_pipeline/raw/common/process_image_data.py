...
from dataset_pipeline.raw.common.process_raw_signals import _copy_signals, get_signals
        ...
        for block_num, xdf_file in enumerate(("block_1.xdf", "block_2.xdf")):
            ...
            for stream in streams:
                ...
                for signal_dict in signals:
                    ...
                    tmp.append({**signal_dict, "timestamp_origin": "lsl", "url": url})
...
def insert_raw_unlabeled_data(
    ...
    with cd(settings.experiment_root_dir):
        ...
        db = next(get_db())
        # Query the table actually being populated, not ScreenCapture — with a
        # hard-coded table, any other image modality run through this function
        # would skip sessions based on screen-capture coverage.
        processed_group_sessions = set(
            [
                s[0]
                for s in db.query(image_table_class.group_session_id)
                .distinct(image_table_class.group_session_id)
                .all()
            ]
        )
        ...
        for group_session in tqdm(sorted(directories_to_process), unit="directories"):
            ...
            if image_records:
                info(f"Inserting {len(image_records):,} rows via COPY.")
                _copy_signals(image_table_class, image_records)
        db.close()
