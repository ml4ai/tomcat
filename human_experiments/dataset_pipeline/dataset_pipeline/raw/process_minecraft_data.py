...
from dataset_pipeline.common.config import LOG_DIR, configure_logging, settings
...
from dataset_pipeline.database.entity.task.minecraft_task import (
    MinecraftMission,
...
def process_metadata_file(
    ...
    # Header timestamps in trial metadata files can be stale: exp_2023_01_31_14's
    # Training trial carried 2022-10-25 stamps in a 2023-01-31 session, which
    # produced a mission row whose stop predated its start by three months. That
    # inverted window made labeling match zero rows, and 24,907 screen captures
    # were then deleted as unlabeled — silently. Refuse to ingest such a row.
    if convert_iso8601_timestamp_to_unix(
        mission_stop_timestamp
    ) <= convert_iso8601_timestamp_to_unix(mission_start_timestamp):
        raise ValueError(
            f"Mission window for trial {trial_id} in {group_session} is empty or "
            f"inverted: start={mission_start_timestamp}, "
            f"stop={mission_stop_timestamp}. A stale header timestamp in the "
            "trial metadata is the known cause; fix the source or add the trial "
            "to INVALID_MISSIONS."
        )
...
def process_minecraft_data():
    configure_logging(f"{LOG_DIR}/build_testbed_message_table.log")
