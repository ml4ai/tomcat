...
from dataset_pipeline.database.entity.base.modality import Modality
from dataset_pipeline.database.entity.base.participant import Participant
from dataset_pipeline.database.entity.base.post_game_survey import (
    PostGameSurvey,
)
...
def process_post_game_survey():
    ...
    for i, row in tqdm(df.iterrows(), total=len(df)):
        ...
        # Idempotency: skip if this participant's survey row is already stored.
        already_stored = db.scalars(
            select(PostGameSurvey).where(
                PostGameSurvey.participant_id == participant_id
            )
        ).first()
        ...
        # post_game_survey.participant is a foreign key to participant; skip rows whose
        # subject is not in the participant table to avoid a foreign-key violation.
        participant_exists = db.scalars(
            select(Participant).where(Participant.id == participant_id)
        ).first()
        if participant_exists is None:
            info(f"Participant ID: {participant_id} not in participant table, skipping survey row...")
            continue
...
def process_base_tables():
    configure_logging(f"{LOG_DIR}/build_base_tables.log")
