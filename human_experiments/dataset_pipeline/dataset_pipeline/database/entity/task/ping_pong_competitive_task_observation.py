...
class PingPongCompetitiveTaskObservation(Base):
    ...
    player_1_station_id: Mapped[str] = mapped_column(
        "player_1_station", Text, ForeignKey(Station.id), primary_key=True
    )
    player_2_station_id: Mapped[str] = mapped_column(
        "player_2_station", Text, ForeignKey(Station.id), primary_key=True
    )
    ...
    player_1_score: Mapped[int] = mapped_column(
        Integer, CheckConstraint("player_1_score >= 0")
    )
    player_2_score: Mapped[int] = mapped_column(
        Integer, CheckConstraint("player_2_score >= 0")
    )
