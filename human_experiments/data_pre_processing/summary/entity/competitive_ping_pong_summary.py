...
class CompetitivePingPongSummary(TaskSummary):
    def __init__(
        self,
        participant_id_left: str,
        participant_id_right: str,
        score_left: str,
        score_right: str,
        team_id: int,
    ):
    ...
    def from_experiment_directory(
        cls, experiment_dir: str, team_id: int
        ...
        game_files = sorted(
            list(
                glob(
                    f"{experiment_dir}/baseline_tasks/ping_pong/competitive_{team_id - 1}_*.csv"
                )
            )
        )
        ...
        if len(game_files) == 0:
            ...
            try:
                ...
                if "score_left" not in game_df.columns:
                    ...
                    return cls(
                        participant_id_left=game_df.columns[-4].split("_")[
                            0
                        ],  # Column named <participant_id1>_y
                        participant_id_right=game_df.columns[-3].split("_")[
                            0
                        ],  # Column named <participant_id2>_x
