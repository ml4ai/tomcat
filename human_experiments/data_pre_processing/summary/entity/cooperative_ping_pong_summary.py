...
class CooperativePingPongSummary(TaskSummary):
    ...
    def from_experiment_directory(
        ...
        # recorded.
        game_files = sorted(
            list(glob(f"{experiment_dir}/baseline_tasks/ping_pong/cooperative_0_*.csv"))
        )
    ...
    def empty_summary(cls) -> CooperativePingPongSummary:
        return cls(team_score=MISSING_INFO, ai_score=MISSING_INFO)

    def to_data_frame(self) -> pd.DataFrame:
        header = [f"Team Score", f"AI Score"]
        # Add info about the team to the header
        header = [f"{h} ({self.task_name})" for h in header]

        data = [self.team_score, self.ai_score]
