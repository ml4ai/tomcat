...
def standardize_silence_periods(
    experiments_dir: str, out_dir: str, silence_threshold: int, override: bool
):
...
def process_directory_v1(
    experiment_dir: str, out_dir: str, silence_threshold: int, override: bool
):
...
def process_directory(
    ...
    for station in ["lion", "tiger", "leopard"]:
        ...
        for annotation_file in os.listdir(annotation_dir):
            if (
                annotation_file[annotation_file.rfind(".") :].lower() != ".textgrid"
                or annotation_file[0] == "."
            ):
                continue

            # Add the threshold to the directory name to be created to store the standardized annotations.
            sub_dir = (
                audio_dir[audio_dir.find("exp_") :]
                + f"/standardized_silences_{silence_threshold}s"
            )
            ...
            if os.path.exists(standardized_annotation_filepath) and not override:
                info(
                    f"Skipping file {annotation_file}. Standardized annotations already found in {out_dir}."
                )
                continue
...
if __name__ == "__main__":
    ...
    standardize_silence_periods(
        args.experiments_dir, args.out_dir, args.silence_threshold, args.override
    )
