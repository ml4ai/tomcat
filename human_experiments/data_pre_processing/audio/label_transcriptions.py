...
from audio.entity.sentence_labeler import SentenceLabeler, ToMCATDialogAgent
from common.config import EXP_DIR, LOG_DIR, OUT_DIR
from utils import is_directory_with_unified_xdf_files


def label_transcriptions(
    experiments_dir: str, out_dir: str, labeler: SentenceLabeler, override: bool
):
    ...
    for station in ["lion", "tiger", "leopard"]:
        ...
        for annotation_file in os.listdir(transcriptions_annotation_dir):
            ...
            if os.path.exists(labels_filepath) and not override:
                info(
                    f"Skipping file {annotation_file}. Label annotations already found in {out_dir}."
                )
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=False,
        default=OUT_DIR,
        help="Directory where experiment folder structure containing semantic labels must be saved.",
    )
    parser.add_argument(
        "--log_dir",
        type=str,
    ...
    label_transcriptions(
        args.experiments_dir, args.out_dir, dialog_agent, args.override
    )
