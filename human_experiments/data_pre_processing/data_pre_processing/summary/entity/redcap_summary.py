...
class TeamREDCapSummary(REDCapSummary):
    def __init__(
        ...
        experiment_date: str,
        team_number: str,
        participants_issues_details: str,
        equipment_issues_details: str,
    ...
    def from_experiment_directory(cls, experiment_dir: str) -> TeamREDCapSummary:
        ...
        redcap_dirs = [
            dir_name
            for dir_name in os.listdir(f"{experiment_dir}")
            if "redcap" in dir_name.lower()
        ]
        ...
        if len(redcap_dirs) > 0:
            try:
                ...
                redcap_filenames = [
                    filename
                    for filename in os.listdir(redcap_dir)
                    if "team_data" in filename.lower()
                ]
                ...
                if len(redcap_filenames) == 0:
                    ...
                    return cls(
                        ...
                        participants_issues_details=redcap_df.iloc[0][
                            "participants_issues_details"
                        ],
                        equipment_issues_details=redcap_df.iloc[0][
                            "equipment_issues_details"
                        ],
...
class ParticipantREDCapSummary(REDCapSummary):
    ...
    def from_experiment_directory(
        cls, experiment_dir: str, machine_name: str
    ) -> ParticipantREDCapSummary:
        # Name of the directory containing REDCap data. We look for it in a list because the case changed over time.
        redcap_dirs = [
            dir_name
            for dir_name in os.listdir(f"{experiment_dir}/{machine_name}")
            if "redcap" in dir_name.lower()
        ]
        ...
        if len(redcap_dirs) > 0:
            ...
            redcap_filenames = [
                filename
                for filename in os.listdir(redcap_dir)
                if "post_game_survey" in filename.lower()
            ]
    ...
    def empty_summary(cls) -> ParticipantREDCapSummary:
        return cls(participant_id=MISSING_INFO)

    def to_data_frame(self) -> pd.DataFrame:
        headers = ["Participant ID"]

        data = [self.participant_id]
