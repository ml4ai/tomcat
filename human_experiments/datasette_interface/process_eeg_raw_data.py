...
def insert_raw_unlabeled_data(database_engine, override):
    ...
    with cd("/tomcat/data/raw/LangLab/experiments/study_3_pilot/group"):
        ...
        with Session(database_engine) as database_session:
            processed_group_sessions = set([s[0] for s in database_session.query(EEGRaw.group_session_id).distinct(
                EEGRaw.group_session_id).all()])

            for group_session in tqdm(
                    sorted(directories_to_process), unit="directories"
            ):
                if not override and group_session in processed_group_sessions:
                    info(f"Found saved EEG data for {group_session} in the database. Skipping group session.")
                    continue
...
def get_signals(stream, group_session, station, initial_id):
    ...
    channels = [channel["custom_name"][0].lower() for channel in stream["info"]["desc"][0]["channels"][0]["channel"]]
    ...
    with cd(f"{group_session}/lsl"):
        ...
        for xdf_file in ("block_1.xdf", "block_2.xdf"):
            ...
            for stream in streams:
                ...
                station = stream["info"]["name"][0].split('-')[1].replace('_actiCHamp', '')
...
def create_indices(database_engine, check):
    ...
    idx_timestamp_unix = Index('idx_timestamp_unix', EEGRaw.timestamp_unix)
    idx_timestamp_unix.create(bind=database_engine, checkfirst=check)

    idx_participant = Index('idx_participant', EEGRaw.participant_id)
    idx_participant.create(bind=database_engine, checkfirst=check)

    idx_group_session = Index('idx_group_session', EEGRaw.group_session_id)
    idx_group_session.create(bind=database_engine, checkfirst=check)

    idx_task = Index('idx_task', EEGRaw.group_session_id)
...
def label_data(database_engine, override):
    ...
    with Session(database_engine) as database_session:
        processed_group_sessions = set([s[0] for s in
                                        database_session.query(EEGRaw.group_session_id).distinct(
                                            EEGRaw.group_session_id).filter(
                                            EEGRaw.task_id.is_not(None)).all()])

        validity_rows = database_session.query(DataValidity.group_session_id,
                                               DataValidity.participant_id,
                                               DataValidity.station_id,
                                               DataValidity.task_id).filter_by(modality_id="eeg").all()
        ...
        for row in tqdm(validity_rows):
            ...
            if not override and group_session in processed_group_sessions:
                info(f"All EEG entries for {group_session} are labeled in the database. Skipping group session.")
                continue
...
def remove_invalid_data(database_engine):
    ...
    with Session(database_engine) as database_session:
        invalid_rows = database_session.query(DataValidity.group_session_id,
                                              DataValidity.station_id,
                                              DataValidity.participant_id,
                                              DataValidity.task_id).filter_by(modality_id="eeg",
                                                                              is_valid=False).all()
        ...
        for row in tqdm(invalid_rows):
            ...
            info(
                f"Data for task {task} for {group_session} for eeg"
                f" modality for station {station}/participant {participant_id}"
                " is not valid. We will delete this data from the table."
            )

            delete_invalid_signals(EEGRaw, group_session, station, task, database_session)
