...
def insert_raw_unlabeled_data(override, signal_modality_class, modality_name,
    ...
    with cd("/tomcat/data/raw/LangLab/experiments/study_3_pilot/group"):
        ...
        processed_group_sessions = set(
            [s[0] for s in
             db_session.query(signal_modality_class.group_session_id).distinct(
...
def label_data(signal_modality_class, modality_name):
    ...
    validity_rows = db_session.query(DataValidity.group_session_id,
...
def remove_invalid_data(signal_modality_class, modality_name):
    ...
    db_session = next(get_db())
    invalid_rows = db_session.query(DataValidity.group_session_id,
                                    DataValidity.station_id,
                                    DataValidity.participant_id,
                                    DataValidity.task_id).filter_by(
