...
def label_signals_with_task(signal_modality_class, task, station, participant_id, start_timestamp,
                            stop_timestamp,
                            database_session):
    stmt = (
...
def label_rest_state_task_data(signal_modality_class, group_session, station, participant_id,
                               database_session):
    start_timestamp, stop_timestamp = database_session.query(
        RestStateTask.start_timestamp_unix,
...
def label_affective_task_individual_data(signal_modality_class, group_session, station,
                                         participant_id,
                                         database_session):
    start_timestamp, stop_timestamp = database_session.query(
...
def label_affective_task_team_data(signal_modality_class, group_session, station, participant_id,
                                   database_session):
    start_timestamp, stop_timestamp = database_session.query(
        func.min(AffectiveTaskEvent.timestamp_unix),
        func.max(AffectiveTaskEvent.timestamp_unix)).filter(
...
def label_finger_tapping_data(signal_modality_class, group_session, station, participant_id,
                              database_session):
    start_timestamp, stop_timestamp = database_session.query(
...
def label_ping_pong_competitive_data(signal_modality_class, group_session, station, participant_id,
                                     database_session):
    start_timestamp, stop_timestamp = database_session.query(
...
def label_ping_pong_cooperative_data(signal_modality_class, group_session, station, participant_id,
                                     database_session):
    start_timestamp, stop_timestamp = database_session.query(
...
def label_minecraft_data(signal_modality_class, group_session, station, participant_id, task,
                         database_session):
    if task == "saturn_a":
        mission = "Saturn_A"
    elif task == "saturn_b":
...
def label_signals(signal_modality_class, group_session, task, station, participant_id,
                  database_session):
    if task == "rest_state":
