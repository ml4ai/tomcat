...
def process_ping_pong_competitive_task_data():
    ...
    with cd("/tomcat/data/raw/LangLab/experiments/study_3_pilot/group"):
        ...
        processed_group_sessions = set(
            ([s[0] for s in
              db_session.query(PingPongCompetitiveTaskObservation.group_session_id).distinct(
...
def recreate_ping_pong_competitive_observation_tables():
    PingPongCompetitiveTaskObservation.__table__.drop(engine, checkfirst=True)
    PingPongCompetitiveTaskObservation.__table__.create(engine, checkfirst=True)
