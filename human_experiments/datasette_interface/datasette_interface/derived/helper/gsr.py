...
class GSRHelper(EEGHelper):

    def __init__(self):
        """
        Creates a GSR modality helper.
        """
        super().__init__()

    def load_data(self, group_session: str):
        """
        Reads GSR data to the memory for a specific group session.
        ...
        self._data = pd.read_sql(
            select(EEGRaw.group_session_id,
                   ...
                   EEGRaw.aux_gsr).where(EEGRaw.group_session_id == group_session),
            db_session)
        db_session.close()
