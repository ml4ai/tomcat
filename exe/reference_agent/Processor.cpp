...
// Update the header and message structs for the heartbeat message
void Processor::update_heartbeat_message(json::object header,
                                         json::object msg) {

    heartbeat_header = new_header(header, "not_set", heartbeat_type);
    heartbeat_msg = new_msg(msg, "not_set", heartbeat_sub_type);
...
void Processor::process_message(const std::string topic,
    ...
    if ((trial_topic == topic) && (trial_type == type)) {

        // Trial start
        if (trial_sub_type_start == sub_type) {
            std::cout << "Trial started" << std::endl;

            // publish version info message
            json::object version_info_message = {
                { "header", new_header(header, ts, version_info_type) },
                { "msg", new_msg(msg, ts, version_info_sub_type) },
                { "data", {
            ...
            update_heartbeat_message(header, msg);
        }

        // Trial stop
	else if (trial_sub_type_stop == sub_type) {
    ...
    else if ((rollcall_request_topic == topic) &&
             (rollcall_request_type == type) &&
             (rollcall_request_sub_type == sub_type)) {
	...
	std::string rollcall_id = val<std::string>(data, "rollcall_id");

        // publish rollcall response message
        json::object rollcall_response_message = {
            { "header", new_header(header, ts, rollcall_response_type) },
            { "msg", new_msg(msg, ts, rollcall_response_sub_type) },
            { "data", {
                { "version", agent_version },
                { "uptime", uptime },
                { "status", "up" },
