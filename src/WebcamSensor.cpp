...
namespace tomcat {
  ...
  void WebcamSensor::get_observation() {
    ...
    //.RecorderOpenFaceParameters
      Utilities::RecorderOpenFaceParameters recording_params(this->arguments, true, this->sequence_reader.IsWebcam(),
      this->sequence_reader.fx, this->sequence_reader.fy, this->sequence_reader.cx, this->sequence_reader.cy, this->sequence_reader.fps);
      
      
      
    // .Create open_face_rec
      Utilities::RecorderOpenFace open_face_rec(this->sequence_reader.name, recording_params, this->arguments);
    ...
    if (detection_success && this->face_model.eye_model) {
      ...
      if (recording_params.outputAlignedFaces() || recording_params.outputHOG() || recording_params.outputAUs() || this->visualizer.vis_align || this->visualizer.vis_hog || this->visualizer.vis_aus)
      {
          face_analyser.AddNextFrame(this->rgb_image, this->face_model.detected_landmarks, this->face_model.detection_success, this->sequence_reader.time_stamp,
    ...
    std::cout<<"Postprocessing the Action Unit predictions\n";
      json j;
      j={
