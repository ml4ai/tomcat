...
int main(int argc, const char* argv[]) {
  ...
  desc.add_options()
      ...
      ("time_limit", value<unsigned int>()->default_value(20),
           ...
           "Self-report prompt interval time (in seconds).")("port,p",
  ...
  if (vm.count("mission")) {
    ...
    unsigned  int selfReportPromptTimeInSeconds = vm["self_report"].as<unsigned int>();
    agent.setMission(missionIdOrPathToXML, timeLimitInSeconds, selfReportPromptTimeInSeconds, width, height, activateVideo, activateObsRec);
