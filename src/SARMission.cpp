...
//        this->drawStairs(40, 3, HEIGHT_OF_GROUND_LEVEL, 4, 5, orientation::east_west);
    ...
    void SARMission::drawTomcatSign(){
        ...
        // Letter T
        this->drawPlane(tomcatX, tomcatZ, tomcatY, 1, 2*FLOOR_HEIGHT, 1, tomcatBlockType);
        ...
        this->drawWall(tomcatX, tomcatZ, tomcatY, tomcatWidth, 2*FLOOR_HEIGHT, orientation::west_east, tomcatBlockType);
        this->makeHole(tomcatX - 1, tomcatZ, tomcatY + 1, tomcatWidth - 2, 2*FLOOR_HEIGHT - 2, 1);

        // Letter M
        tomcatX = tomcatX - tomcatWidth - 1;
        this->drawPlane(tomcatX, tomcatZ, tomcatY, 1, 2*FLOOR_HEIGHT, 1, tomcatBlockType);
        this->drawPlane(tomcatX - 1 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 1, 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - 2 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 2, 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - 3 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 3, 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - 4 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 2, 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - 5 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 1, 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - tomcatWidth + 1, tomcatZ, tomcatY, 1, 2*FLOOR_HEIGHT, 1, tomcatBlockType);

        // Letter C
        tomcatX = tomcatX - tomcatWidth - 1;
        this->drawWall(tomcatX, tomcatZ, tomcatY, tomcatWidth, 2*FLOOR_HEIGHT, orientation::west_east, tomcatBlockType);
        this->makeHole(tomcatX - 1, tomcatZ, tomcatY + 1, tomcatWidth, 2*FLOOR_HEIGHT - 2, 1);

        // Letter A
        tomcatX = tomcatX - tomcatWidth - 1;
        this->drawPlane(tomcatX, tomcatZ, tomcatY, 1, 2*FLOOR_HEIGHT, 1, tomcatBlockType);
        this->drawPlane(tomcatX -1 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 1, tomcatWidth - 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX -1 , tomcatZ, tomcatY + 2*FLOOR_HEIGHT - 6, tomcatWidth - 1, 1, 1, tomcatBlockType);
        this->drawPlane(tomcatX - tomcatWidth + 1, tomcatZ, tomcatY, 1, 2*FLOOR_HEIGHT, 1, tomcatBlockType);
    ...
    void SARMission::drawGroundFloor() {
        this->drawRoom(BUILDING_SOUTHWEST_X_POSITION, BUILDING_SOUTHWEST_Z_POSITION, HEIGHT_OF_GROUND_LEVEL, BUILDING_WIDTH,
                FLOOR_HEIGHT, BUILDING_DEPTH);
        // Make hole in the wall for the main entrance of the floor
        this->makeHole(BUILDING_SOUTHWEST_X_POSITION - 1, BUILDING_SOUTHWEST_Z_POSITION, HEIGHT_OF_GROUND_LEVEL,
                MAIN_ENTRANCE_WIDTH, FLOOR_HEIGHT - 1, 1);
        this->drawStairs(BUILDING_SOUTHWEST_X_POSITION - 1, BUILDING_SOUTHWEST_Z_POSITION + BUILDING_DEPTH - FLOOR_HEIGHT - 1,
                HEIGHT_OF_GROUND_LEVEL, STAIRS_WIDTH, FLOOR_HEIGHT, south_north);
        // Make hole in the roof to open an access to the next floor
        this->makeHole(BUILDING_SOUTHWEST_X_POSITION - 1, BUILDING_SOUTHWEST_Z_POSITION + BUILDING_DEPTH - FLOOR_HEIGHT - 1,
                HEIGHT_OF_GROUND_LEVEL + FLOOR_HEIGHT - 1,  STAIRS_WIDTH, 1, FLOOR_HEIGHT - 1);
        // Make holes in the lateral walls to receive light from outside
        this->makeHole(BUILDING_SOUTHWEST_X_POSITION - BUILDING_WIDTH - 1, BUILDING_SOUTHWEST_Z_POSITION + 2,
                 HEIGHT_OF_GROUND_LEVEL + 2,  1, 2, BUILDING_DEPTH - 4);
        this->makeHole(BUILDING_SOUTHWEST_X_POSITION, BUILDING_SOUTHWEST_Z_POSITION + 2,
                 HEIGHT_OF_GROUND_LEVEL + 2,  1, 2, BUILDING_DEPTH - FLOOR_HEIGHT - 4);
    }
