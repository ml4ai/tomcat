...
namespace tomcat {
    namespace model {
        ...
        TomcatTA3V2::get_nodes() const {
            ...
            // Theta_PBAE
            shared_ptr<NodeMetadata> theta_pbae_metadata =
                this->create_theta_pbae_metadata();
            shared_ptr<CPD> theta_pbae_prior_cpd =
                this->create_theta_pbae_prior_cpd();
            nodes[THETA_PBAE] =
                this->create_node(theta_pbae_metadata, {theta_pbae_prior_cpd});
            ...
            shared_ptr<CPD> pbae_prior_cpd = this->create_pbae_prior_cpd(nodes[THETA_PBAE]);
            nodes[PBAE] = this->create_node(pbae_metadata, {pbae_prior_cpd});
        ...
        shared_ptr<NodeMetadata> TomcatTA3V2::create_theta_q_metadata() const {
            shared_ptr<NodeMetadata> metadata = make_shared<NodeMetadata>(
                NodeMetadata::create_multiple_time_link_metadata(
                    THETA_Q, false, true, false, 0, NUM_TRAINING_CONDITIONS));

            return metadata;
        }

        shared_ptr<NodeMetadata>
        TomcatTA3V2::create_theta_pbae_metadata() const {
            shared_ptr<NodeMetadata> metadata = make_shared<NodeMetadata>(
                NodeMetadata::create_multiple_time_link_metadata(
                    THETA_PBAE, false, true, false, 0, NUM_BEEP_STATES));
        ...
        shared_ptr<CPD> TomcatTA3V2::create_theta_pbae_prior_cpd() const {
            Eigen::MatrixXd cpd_table =
