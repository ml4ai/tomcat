...
World generateAABBGrid(World world, int N, int sep, int AABB_size, string material){
    int idCtr =1;
    Pos topLeft(1,0,1);
    ...
    while (idCtr <= (N* N -1)){
        ...
        if ((idCtr -1) % N == 0){
            ...
            Pos newBottomRight(AABB_size, AABB_size, newTopLeft.getZ() + AABB_size);
...
World generateGridWorld(int N, int sep, int AABB_size, string AABB_material, string filename){
    World world;
