...
def split_data_files_into_folders(input_folder, output_folder, show_progress=True):
    ...
    for experiment_data_file in tqdm(os.listdir(input_folder), desc='Splitting files into folders',
        ...
        if os.path.isdir(os.path.join(input_folder, experiment_data_file)) or experiment_data_file.startswith('.'):
            continue
                ...
                if data_point.strip():
                    ...
                    if message['header']['message_type'] == 'event':
                        if message['msg']['sub_type'] == 'Event:Triage':
                            ...
                            if message['data']['mission_state'].upper() == 'START':
                                mission_start_time = to_datetime(message['header']['timestamp'])
                                metadata.append({'start_time': message['header']['timestamp']})
        ...
        write_dataset_to_file(metadata, os.path.join(experiment_folder, METADATA_FILENAME))
...
def extract_room_events_from_observations(formatted_experiment_data_folder, mission_map_id, show_progress=True):
    for experiment_folder in tqdm(os.listdir(formatted_experiment_data_folder),
        ...
        observations = read_data_from_file(
            os.path.join(formatted_experiment_data_folder, experiment_folder, OBSERVATIONS_FILENAME))
        previous_area_id = get_initial_area(mission_map_id)
        areas = get_list_of_areas(mission_map_id)

        room_events = []
        for observation in observations:
            new_area_id = get_area_id_by_players_position(areas, observation['data']['x'], observation['data']['z'])
            if new_area_id != previous_area_id and new_area_id != None:
                ...
                area_event_message['topic'] = 'observations/events/player/location'
                area_event_message['header'] = {'version': 0.5, 'message_type': 'event',
                                                'timestamp': observation['header']['timestamp']}
                area_event_message['data'] = {'playername': observation['data']['name'], 'entered_area_id': new_area_id,
                                              'exited_area_id': previous_area_id}

                room_events.append(area_event_message)
                previous_area_id = new_area_id

        write_dataset_to_file(room_events,
                              os.path.join(formatted_experiment_data_folder, experiment_folder, ROOM_FILENAME))
...
def get_area_id_by_players_position(areas, player_x, player_y):
    for area in areas:
        if area['x1'] <= player_x <= area['x2'] and area['y1'] <= player_y <= area['y2']:
            return area['id']
